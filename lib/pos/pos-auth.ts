...
export const posCashierCookieOptions = {
  httpOnly: true,
...
export const posTerminalCookieOptions = {
  httpOnly: true,
  sameSite: 'lax' as const,
  secure: secureCookie,
  path: '/',
  ...
  return (
    (
      await db
        ...
        .where(
          and(
            eq(posTerminal.tokenHash, tokenHash(token)),
...
export const getPosAuthorizationContext = cache(
  async (): Promise<
    | (AuthorizationContext & {
        authMethod: 'pos_pin';
        branchId: string;
  ...
  > => {
    ...
    const [record] = await db
      ...
      .where(
        and(
          ...
          gt(posAuthSession.expiresAt, new Date())
        )
      )
      .limit(1);
    ...
    const [[account], [membership], [assigned], [staff]] = await Promise.all([
      ...
      db
        ...
        .where(
          and(
            eq(organizationMembership.organizationId, record.organizationId),
            eq(organizationMembership.userId, record.userId)
          )
        )
        .limit(1),
      db
        ...
        .where(
          and(
            eq(branchMembership.branchId, record.branchId),
            eq(branchMembership.userId, record.userId)
          )
        )
        .limit(1),
      db
        ...
        .where(
          and(
            eq(employee.orgId, record.organizationId),
            eq(employee.userId, record.userId)
          )
        )
        .limit(1),
    ]);
...
export async function requirePointOfSaleAuthorization(
  permission: PermissionEnum
) {
  const context = await getPosAuthorizationContext();
  if (!context || !context.permissions.includes(permission))
    throw new AuthorizationError('POS is locked or access is not permitted');
  return context;
}
