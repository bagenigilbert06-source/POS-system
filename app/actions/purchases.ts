...
const supplierSchema = z.object({
  ...
  paymentTermsDays: z.coerce.number().int().nonnegative().max(365).default(0),
  leadTimeDays: z.coerce.number().int().nonnegative().max(365).default(0),
...
export async function getProcurementData() {
  ...
  const [
    ...
    db
      .select()
      .from(purchase)
      .where(eq(purchase.orgId, orgId))
      .orderBy(desc(purchase.createdAt))
      .limit(100),
    db.select().from(purchaseItem).where(eq(purchaseItem.orgId, orgId)),
    db
      .select()
      .from(purchaseReceipt)
      .where(eq(purchaseReceipt.orgId, orgId))
      .orderBy(desc(purchaseReceipt.createdAt))
      .limit(100),
    db
      .select()
      .from(product)
...
export async function updateSupplier(
  ...
  const supplierId = z.string().min(1).parse(id),
    data = supplierSchema.parse(input);
...
export async function setSupplierStatus(
  ...
  const supplierId = z.string().min(1).parse(id),
    next = z.enum(['active', 'inactive']).parse(status);
  const { orgId } = await context(PermissionEnum.PURCHASE_MANAGE);
  const [updated] = await db
    .update(supplier)
...
export async function upsertSupplierProduct(
  input: z.input<typeof supplierProductSchema>
) {
  const data = supplierProductSchema.parse(input),
    { orgId } = await context(PermissionEnum.PURCHASE_MANAGE);
  const [[vendor], [item]] = await Promise.all([
    db
      .select({ id: supplier.id })
      .from(supplier)
      .where(and(eq(supplier.id, data.supplierId), eq(supplier.orgId, orgId)))
      .limit(1),
    db
  ...
  await db.transaction(async (tx) => {
    if (data.isPreferred)
      await tx
        ...
        .where(
          and(
            eq(supplierProduct.productId, data.productId),
            eq(supplierProduct.orgId, orgId)
          )
        );
    await tx
      ...
      .values({
        ...
        minimumOrderQuantity: String(data.minimumOrderQuantity),
        packSize: String(data.packSize),
        orgId,
      })
      .onConflictDoUpdate({
    ...
    if (data.isPreferred)
      await tx
        .update(product)
        .set({ preferredSupplierId: data.supplierId, updatedAt: new Date() })
        .where(and(eq(product.id, data.productId), eq(product.orgId, orgId)));
  });
  revalidatePath('/dashboard/purchases');
  revalidatePath('/dashboard/inventory');
...
export async function recordPurchasePayment(
  ...
  await db.transaction(async (tx) => {
    await tx.execute(
      sql`select ${purchase.id} from ${purchase} where ${purchase.id} = ${data.purchaseId} for update`
    );
    const [record] = await tx
      .select()
      .from(purchase)
      .where(and(eq(purchase.id, data.purchaseId), eq(purchase.orgId, orgId)))
      .limit(1);
    if (!record) throw new Error('Purchase was not found');
    if (record.status === 'voided')
      throw new Error('A voided purchase cannot be paid');
    ...
    const paidAmount = Number(record.paidAmount) + data.amount;
    const paid = paidAmount >= Number(record.total) - 0.001;
    const note = data.reference
    ...
    await tx
      ...
      .where(and(eq(purchase.id, record.id), eq(purchase.orgId, orgId)));
  });
  revalidatePath('/dashboard/purchases');
...
export async function receivePurchase(input: z.input<typeof receiptSchema>) {
  ...
  if (
    ...
    throw new Error('You do not have access to this receiving location');
  await db.transaction(async (tx) => {
    const [[vendor], [item]] = await Promise.all([
      tx
        .select()
    ...
    if (item.trackingMode !== 'none')
      throw new Error(
        'Receive lot- or serial-tracked products through a confirmed purchase order'
      );
    ...
    await tx
      ...
      .values({
        ...
        total: String(total),
        paidAmount: String(data.amountPaid),
        paymentStatus,
    ...
    await applyInventoryMovement(tx, {
      ...
      reason: data.reference || purchaseNo,
      userId,
      orgId,
    ...
    await tx
      .update(product)
      .set({ buyingPrice: String(data.unitCost), updatedAt: new Date() })
