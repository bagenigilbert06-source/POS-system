...
import {
  ...
  consumeCafeRecipeInventory,
  getCafeConfiguration,
  resolveCafeCheckout,
  type CafeCheckoutInput,
} from '@/lib/cafe/sale-service';
...
async function getOrgId(userId: string, moduleId = 'sales') {
  ...
  if (!config?.enabledModules.includes(moduleId))
    throw new Error(`${moduleId} is not enabled for this workspace`);
...
async function syncLinkedInvoiceFiscalStatus(saleId: string, orgId: string) {
  const [fiscal] = await db
    ...
    .where(
      and(
        eq(etimsSubmission.saleId, saleId),
        eq(etimsSubmission.organizationId, orgId)
      )
    )
    .limit(1);
  if (!fiscal) return;
  await db
    .update(invoice)
    .set({
      fiscalStatus: fiscal.status.toLowerCase(),
      fiscalReference: fiscal.invoiceNumber || fiscal.controlNumber || null,
      updatedAt: new Date(),
    })
    .where(and(eq(invoice.saleId, saleId), eq(invoice.orgId, orgId)));
}
...
const offlineMetadataSchema = z.object({
  ...
  provisionalReceiptNo: z
    .string()
    .trim()
    .regex(/^OFF-[A-Z0-9-]{8,40}$/),
...
export async function startAgeVerification(
  input: z.input<typeof startAgeVerificationSchema>
) {
...
export async function cancelAgeVerification(
  input: z.input<typeof cancelAgeVerificationSchema>
) {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx
      ...
      .values({
        ...
        organizationId,
        userId,
        action: 'age_verification_cancelled',
        metadata: { checkoutId, branchId, terminalId: shift.terminalId },
      });
  });
...
export async function voidSale(input: z.input<typeof voidSaleSchema>) {
  ...
  await db.transaction(async (tx) => {
    ...
    const [fiscal] = await tx
      .select({ id: etimsSubmission.id, status: etimsSubmission.status })
      .from(etimsSubmission)
      .where(eq(etimsSubmission.saleId, record.id))
      .limit(1);
    if (fiscal && ['ACCEPTED', 'CREDITED'].includes(fiscal.status))
      throw new Error(
        'This sale has an accepted eTIMS invoice. Process a refund/credit note instead of voiding it.'
      );
    if (fiscal && ['SUBMITTING', 'RETRYING'].includes(fiscal.status))
      throw new Error(
        'This sale has an eTIMS submission in progress. Reconcile it before voiding the sale.'
      );
    ...
    if (!['completed', 'pending'].includes(record.status))
      throw new Error('Only completed or pending sales can be voided');
    const [existingReturn] = await tx
      .select({ id: salesReturn.id })
      .from(salesReturn)
      .where(
        and(
          eq(salesReturn.saleId, record.id),
    ...
    const lines = await tx
      .select()
      .from(saleItem)
      .where(and(eq(saleItem.saleId, record.id), eq(saleItem.orgId, orgId)));
    const cafeMetadata = lines.length
      ? await tx
          .select({
    ...
    const cafeInventoryMode = new Map(
      cafeMetadata.map((item) => [item.productId, item.inventoryMode])
    );
    if (record.status === 'completed')
      for (const line of lines) {
        ...
        await applyInventoryMovement(tx, {
          ...
          userId,
          orgId,
          unitCost: Number(line.unitCostAtSale),
        });
      }
    if (record.status === 'completed')
      await reverseRewardsForVoid(tx, {
    ...
    const [linkedCafeOrder] = await tx
      .select({ id: cafeOrder.id, tableId: cafeOrder.tableId })
      .from(cafeOrder)
      .where(
        and(
          eq(cafeOrder.organizationId, orgId),
          eq(cafeOrder.saleId, record.id)
        )
      )
      .limit(1);
    if (linkedCafeOrder) {
      await tx
        ...
        .where(eq(cafeOrder.id, linkedCafeOrder.id));
      if (linkedCafeOrder.tableId)
        await tx
          .update(cafeTable)
          .set({ status: 'available', updatedAt: new Date() })
          .where(eq(cafeTable.id, linkedCafeOrder.tableId));
    }
    if (fiscal && ['PENDING', 'FAILED'].includes(fiscal.status))
      await tx
        .update(etimsSubmission)
        .set({
          status: 'CANCELLED',
          nextRetryAt: null,
          errorMessage: 'Sale voided before fiscal acceptance',
          updatedAt: new Date(),
        })
        .where(eq(etimsSubmission.id, fiscal.id));
    await tx
      .insert(auditEvent)
      .values({
        id: generateId(),
        ...
        metadata: {
          ...
          etimsStatus: fiscal?.status ?? null,
        },
      });
  });
...
export async function createManualSale(
  input: z.input<typeof manualSaleSchema>
) {
  ...
  const [settings] = await db
    ...
    .limit(1);
  const allowedMethods = Array.isArray(settings?.paymentMethods)
    ? (settings.paymentMethods as string[])
    : [];
  if (!allowedMethods.includes(data.paymentMethod))
    throw new Error('Choose a payment method enabled for this workspace');
  const taxAmount = configuredTax(data.amount, {
    enabled: settings?.taxEnabled ?? false,
    ratePercent: Number(settings?.taxRate ?? 0),
  ...
  let branchId = authorization.isOrganizationWide
    ? null
    : authorization.branchIds[0];
  if (authorization.isOrganizationWide) {
    const [mainBranch] = await db
      .select({ id: branch.id })
      .from(branch)
      .where(and(eq(branch.organizationId, orgId), eq(branch.isMain, true)))
      .limit(1);
  ...
  let etims:
    | Awaited<ReturnType<typeof queueEtimsInvoice>>
    | { status: 'PENDING'; message: string };
  try {
    etims = await queueEtimsInvoice(saleId);
  } catch {
    etims = {
      status: 'PENDING',
      message: 'Sale completed. eTIMS submission will require reconciliation.',
    };
  }
...
export async function createSale(data: CreateSaleInput) {
  if (
    ...
    throw new Error('Add between 1 and 250 products');
  if (!Number.isFinite(data.discountAmount) || data.discountAmount < 0)
    throw new Error('Invalid discount amount');
  if (!data.idempotencyKey || data.idempotencyKey.length > 100)
    throw new Error('A valid transaction ID is required');
  const posAuthorization = await getPosAuthorizationContext();
  if (!posAuthorization)
    throw new Error(
      'Unlock this terminal with your POS PIN before completing a sale'
    );
  const userId = posAuthorization.userId;
  const saleAuthorization = posAuthorization;
  if (!saleAuthorization.permissions.includes(PermissionEnum.POS_SELL))
    throw new Error('POS sale permission denied');
  ...
  if (data.cafe && !cafeWorkspace)
    throw new Error('Café order details are not valid for this workspace');
  const idempotencyKey = data.idempotencyKey;
  const offline = data.offline
    ? offlineMetadataSchema.parse(data.offline)
    : null;
  const pharmacyWorkflow = pharmacyWorkflowSchema.parse(data.pharmacy);
  if (offline && data.paymentMethod !== 'cash')
    throw new Error('Offline synchronization supports cash sales only');
  if (offline && offline.queueId !== idempotencyKey)
    throw new Error('Offline queue ID must match the sale transaction ID');
  ...
  if (existingSale) {
    const existingItems = await db
      .select({ saleItemId: saleItem.id, productId: saleItem.productId })
      .from(saleItem)
      .where(
        and(eq(saleItem.saleId, existingSale.id), eq(saleItem.orgId, orgId))
      );
    let etims:
      | Awaited<ReturnType<typeof queueEtimsInvoice>>
      | { status: 'PENDING'; message: string };
    try {
      etims = await queueEtimsInvoice(existingSale.id);
    } catch {
      etims = {
        status: 'PENDING',
        message:
          'Sale completed. eTIMS submission will require reconciliation.',
      };
    }
    if ('submission' in etims && etims.submission)
      schedulePostSaleWork({
        saleId: existingSale.id,
        orgId,
        submissionId: etims.submission.id,
      });
    const [existingCafeOrder] = cafeWorkspace
      ? await db
          .select({
            id: cafeOrder.id,
          ...
          .where(
            and(
              eq(cafeOrder.organizationId, orgId),
              eq(cafeOrder.saleId, existingSale.id)
            )
          )
          .limit(1)
      : [];
  ...
  const [activeShift] = await db
    ...
    .where(
      and(
        ...
        eq(posSession.openedBy, userId),
        offline ? eq(posSession.id, offline.sessionId) : undefined,
        checkoutTerminalId
          ? eq(posSession.terminalId, checkoutTerminalId)
          : undefined,
        eq(posSession.status, 'open')
      )
    )
  ...
  if (!activeShift)
    throw new Error(
      offline
        ? 'The original shift is closed or unavailable. This offline sale requires manager reconciliation.'
        : 'Start your shift before completing a sale'
    );
  if (
    offline &&
    (offline.createdAt.getTime() < activeShift.openedAt.getTime() - 60_000 ||
      offline.createdAt.getTime() > Date.now() + 5 * 60_000)
  )
    throw new Error('Offline sale time falls outside the original shift');
  ...
  if (!saleBranchId)
    throw new Error('No authorized branch is available for this sale');
  const [fiscalConfiguration] = await db
    .select({
      environment: etimsConfiguration.environment,
      enabled: etimsConfiguration.enabled,
      invoiceSubmissionEnabled: etimsConfiguration.invoiceSubmissionEnabled,
    ...
    .where(
      and(
        eq(etimsConfiguration.organizationId, orgId),
        eq(etimsConfiguration.branchId, saleBranchId)
      )
    )
    .limit(1);
  ...
  if (
    ...
    fiscalReadiness !== 'DEVELOPMENT_SIMULATOR' &&
    fiscalConfiguration?.environment === 'production'
  )
    throw new Error(
  ...
  if (
    ...
    !z.string().min(1).max(120).safeParse(data.ageVerificationId).success
  )
    throw new Error('Invalid age verification record');
  if (data.ageVerificationId && verificationEvidence)
    throw new Error(
      'Use either a server-issued verification record or inline verification evidence'
    );
  ...
  if (verificationEvidence?.status === 'OVERRIDDEN') {
    if (
      !saleAuthorization.permissions.includes(
        PermissionEnum.AGE_VERIFICATION_OVERRIDE
      )
    )
      throw new Error('Age verification override permission denied');
    if (!verificationEvidence.overrideReason)
      throw new Error('Enter a reason for the age verification override');
  }

  if (
    data.discountAmount > 0 &&
    !saleAuthorization.permissions.includes(PermissionEnum.POS_DISCOUNT)
  )
    throw new Error('A supervisor or manager must apply this discount');
  const productIds = Array.from(
    new Set(data.items.map((line) => line.productId))
  );
  if (!cafeWorkspace && productIds.length !== data.items.length)
    throw new Error('Duplicate products must be combined into one basket line');
  const catalogue = await db
    ...
    .from(product)
    .where(and(eq(product.orgId, orgId), inArray(product.id, productIds)));
  const catalogueById = new Map(catalogue.map((item) => [item.id, item]));
  // This happens before the sale transaction/stock mutation. For async payment
  // methods a provider callback may already have captured funds, so the later
  // outbox guard remains in place and managers reconcile rather than lose money.
  if (
    fiscalConfiguration?.enabled &&
    fiscalConfiguration.providerName === 'KRA_OSCU'
  ) {
    const incomplete = catalogue.filter(
      (item) =>
        !item.etimsItemCode ||
        !item.etimsItemClassificationCode ||
        !item.etimsItemTypeCode ||
        !item.etimsOriginCountryCode ||
        !item.etimsPackagingUnitCode ||
        !item.etimsQuantityUnitCode ||
        !item.etimsTaxCategory ||
        item.etimsRegistrationStatus !== 'REGISTERED'
    );
    if (incomplete.length)
      throw new Error(
        `${incomplete.length} ${incomplete.length === 1 ? 'item requires' : 'items require'} eTIMS configuration before this sale can be fiscalized.`
      );
  }
  ...
  const categoryRestrictions = categoryIds.length
    ? await db
        .select({
          id: category.id,
          requiresAgeVerification: category.requiresAgeVerification,
        })
        .from(category)
        .where(
          and(eq(category.orgId, orgId), inArray(category.id, categoryIds))
        )
    : [];
  const categoryRestrictionById = new Map(
    categoryRestrictions.map((item) => [item.id, item.requiresAgeVerification])
  );
  const requiresAgeVerification = catalogue.some(
    (item) =>
  ...
  const medicineRows = pharmacyWorkspace
    ? await db
        .select()
        ...
        .where(
          and(
            ...
            inArray(pharmacyProduct.productId, productIds)
          )
        )
    : [];
  const [pharmacyPolicy] = pharmacyWorkspace
    ? await db
        .select()
        .from(pharmacyConfiguration)
        .where(eq(pharmacyConfiguration.organizationId, orgId))
        .limit(1)
    : [];
  const prescriptionItems = medicineRows.filter(
  ...
  if (
    ...
    throw new Error(
      'This staff role is not allowed to dispense prescription medicines'
    );
  if (
    (pharmacyPolicy?.prescriptionWorkflowEnabled ?? true) &&
    prescriptionItems.length &&
    !pharmacyWorkflow?.prescriptionReference
  )
    throw new Error('Enter the prescription reference for this sale');
  if (
    (pharmacyPolicy?.restrictedItemWorkflowEnabled ?? true) &&
    ...
    throw new Error(
      'A pharmacist or authorized manager must approve this restricted-item sale'
    );
  if (
    restrictedItems.length &&
    !(pharmacyWorkflow?.approvalReason || pharmacyWorkflow?.notes)?.trim()
  )
    throw new Error('Enter the restricted-medicine approval reason');
  if (pharmacyWorkflow?.expiresAt && pharmacyWorkflow.expiresAt <= new Date())
    throw new Error('The prescription has expired');
  ...
  const defaultCafeCheckout: CafeCheckoutInput | null = cafeWorkspace
    ...
    : null;
  const resolvedCafe = cafeWorkspace
    ? await resolveCafeCheckout({
        organizationId: orgId,
  ...
  for (const [lineIndex, line] of data.items.entries()) {
    ...
    if (!catalogueItem?.active)
      throw new Error('A selected product is unavailable');
    const selectedPackage = line.packageId
      ? packagesById.get(line.packageId)
      : null;
    if (
      line.packageId &&
      (!selectedPackage || selectedPackage.productId !== line.productId)
    )
      throw new Error(
        `The selected package for ${catalogueItem.name} is unavailable`
      );
    const cafeLine = resolvedCafe?.lines[lineIndex];
    if (
      cafeLine &&
      (cafeLine.productId !== line.productId ||
    ...
    if (
      offline &&
      offlineAmountConflicts(Number(line.unitPrice), unitPrice, 0.001)
    )
      throw new Error(
        `Offline price conflict for ${catalogueItem.name}. Review this sale before synchronizing.`
      );
    normalizedItems.push({
      productId: catalogueItem.id,
      productName:
        cafeLine?.displayName ??
        (selectedPackage
          ? `${catalogueItem.name} (${selectedPackage.name})`
          : catalogueItem.name),
      quantity: line.quantity,
      unitPrice,
      ...
      baseUnitQuantity,
      modifierOptionIds: cafeLine?.modifiers.map(
  ...
  const configuredMethods = Array.isArray(settings?.paymentMethods)
    ? (settings.paymentMethods as string[])
    : [];
  const allowedMethods = Array.from(
    new Set([
      ...(configuredMethods.length > 0 ? configuredMethods : ['cash']),
      'airtel_money',
    ])
  );
  if (
  ...
  let paymentReference = (data.paymentReference ?? data.mpesaRef ?? '')
    .trim()
    .slice(0, 120);
  if (data.paymentMethod === 'card' && !data.cardPaymentAttemptId)
    throw new Error('Record the approved physical terminal payment first');
  if (data.paymentMethod === 'bank_transfer' && !paymentReference)
    throw new Error('Enter the bank transfer reference');
  if (data.paymentMethod === 'airtel_money' && !paymentReference)
    throw new Error('Enter the Airtel Money transaction reference');
  if (data.paymentMethod === 'credit' && !data.customerId)
    throw new Error('Select a customer for a credit sale');
  if (
    data.paymentMethod === 'credit' &&
    (!data.creditDueDate || Number.isNaN(data.creditDueDate.getTime()))
  )
    throw new Error('Select a valid due date for the customer credit');

  // Server-side calculation of tax (do not trust client)
  const calculatedTax = configuredTax(serverSubtotal, {
    enabled: settings?.taxEnabled ?? false,
    ratePercent: Number(settings?.taxRate ?? 0),
    pricesIncludeTax: settings?.pricesIncludeTax ?? false,
  }).toNumber();
  ...
  const unroundedTotal = Number(
    (grossBeforeDiscount + shippingAmount - data.discountAmount).toFixed(2)
  );
  ...
  if (data.paymentMethod === 'mpesa') {
    throw new Error(
      'M-Pesa sales are completed automatically by the verified Daraja callback'
    );
  }
  if (offline && offlineAmountConflicts(Number(data.total), calculatedTotal))
    throw new Error(
      'Offline total conflicts with the current tax or pricing configuration. Review this sale before synchronizing.'
    );
  ...
  if (data.paymentMethod === 'cash' && !Number.isFinite(amountReceived))
  ...
  let cafeOrderResult: {
    orderId: string;
    orderNumber: number;
    preparationStatus: 'new' | 'completed';
  } | null = null;
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      if (!lockedShift)
        throw new Error(
          'This shift is no longer open. Start a new shift before completing the sale.'
        );
      ...
      if (requiresAgeVerification && data.ageVerificationId) {
        const [record] = await tx
          ...
          .where(
            and(
              eq(ageVerification.id, data.ageVerificationId),
              eq(ageVerification.organizationId, orgId),
          ...
          .for('update');
        if (!record)
          throw new Error('Age verification does not belong to this checkout');
        linkedVerification = record;
      }
      if ((data.pointsToRedeem || data.bonusToUse) && !data.customerId)
        throw new Error('Select a customer before using rewards');
      if (
        (data.pointsToRedeem || data.bonusToUse) &&
        !saleAuthorization.permissions.includes(PermissionEnum.REWARDS_REDEEM)
      )
        throw new Error('Reward redemption permission denied');
      let lockedCreditLimit: typeof customerCreditLimit.$inferSelect | null =
        null;
      if (data.paymentMethod === 'credit') {
        const [limit] = await tx
          .select()
          .from(customerCreditLimit)
          .where(
            and(
              eq(customerCreditLimit.customerId, data.customerId!),
              eq(customerCreditLimit.orgId, orgId),
              eq(customerCreditLimit.status, 'active')
            )
          )
          .limit(1)
          .for('update');
        if (!limit)
          throw new Error('This customer does not have an active credit limit');
        lockedCreditLimit = limit;
      }
      const rewards = data.customerId
        ? await applySaleRewards(tx, {
            organizationId: orgId,
            ...
            lines: normalizedItems.map((item) => ({
              ...
              categoryId: catalogueById.get(item.productId)?.categoryId ?? null,
              amount: preTaxRewardAmount(item.totalPrice, {
                enabled: settings?.taxEnabled ?? false,
                ratePercent: Number(settings?.taxRate ?? 0),
                pricesIncludeTax: settings?.pricesIncludeTax ?? false,
              }),
              campaignAmount: item.totalPrice,
              discounted: data.discountAmount > 0,
            })),
            ordinaryDiscount: preTaxRewardAmount(data.discountAmount, {
              enabled: settings?.taxEnabled ?? false,
              ratePercent: Number(settings?.taxRate ?? 0),
              pricesIncludeTax: settings?.pricesIncludeTax ?? false,
            }),
        ...
        : null;
      const rewardAdjustedUnrounded = money(
        unroundedTotal - (rewards?.externalAmountReduction ?? 0)
      );
      const rewardAdjustedMpesa = calculateMpesaAmount(rewardAdjustedUnrounded);
      calculatedTotal = appliesRoundoff
        ? rewardAdjustedMpesa.amount
      ...
      if (
        lockedCreditLimit &&
        financeMoney(
          new Decimal(lockedCreditLimit.currentBalance).plus(calculatedTotal)
        ).greaterThan(financeMoney(lockedCreditLimit.creditLimit))
      )
      ...
      let approvedCardAttempt: typeof cardPaymentAttempt.$inferSelect | null =
        null;
      if (data.paymentMethod === 'card') {
        const [attempt] = await tx
          .select()
          .from(cardPaymentAttempt)
          .where(
            and(
        ...
        if (!attempt)
          throw new Error(
            'The approved card payment does not belong to this checkout'
          );
        if (attempt.status === 'completed' && attempt.saleId)
          throw new Error('This card approval has already been used');
        if (attempt.status !== 'approved_pending_sale')
          throw new Error(
            'This card approval requires reconciliation and cannot be reused'
          );
        const [terminal] = await tx
          .select({ id: cardTerminal.id })
          .from(cardTerminal)
          .where(
            and(
              eq(cardTerminal.id, attempt.cardTerminalId),
              eq(cardTerminal.organizationId, orgId),
              eq(cardTerminal.branchId, saleBranchId),
              eq(cardTerminal.isActive, true)
            )
          )
          .limit(1);
        if (!terminal)
          throw new Error(
            'The selected physical card terminal is no longer active'
          );
        if (Math.abs(Number(attempt.amount) - calculatedTotal) > 0.009)
          throw new Error(
            `Terminal approval amount does not match the sale total of ${calculatedTotal.toFixed(2)}`
          );
      ...
      for (const [itemIndex, item] of saleItems.entries()) {
        ...
        const inventoryQuantity = baseUnitsForSale(
          item.quantity,
        ...
        if (movement.lotAllocations.length)
          lotsBySaleItem.set(item.saleItemId, movement.lotAllocations);
        costBySaleItem.set(
          item.saleItemId,
      ...
      if (resolvedCafe) {
        const recipeCosts = await consumeCafeRecipeInventory(tx, {
          organizationId: orgId,
          branchId: saleBranchId,
          userId,
          saleId,
          receiptNo,
          checkout: resolvedCafe,
          saleItemIds: saleItems.map((item) => item.saleItemId),
        });
        for (const [saleItemId, cost] of recipeCosts)
          costBySaleItem.set(saleItemId, cost);
      }
      ...
      await tx.insert(sale).values({
        ...
        rewardEarningRateSnapshot: rewards
          ? String(rewards.settings.spendPerPoint)
          : null,
        rewardPointValueSnapshot: rewards
          ? String(rewards.settings.pointValue)
          : null,
        total: String(calculatedTotal),
        amountReceived:
          data.paymentMethod === 'cash' ? String(amountReceived) : null,
        ...
        ageVerifiedAt:
          verificationEvidence || linkedVerification ? new Date() : null,
        ageVerifiedBy:
          verificationEvidence || linkedVerification ? userId : null,
        status: 'completed',
        idempotencyKey,
      ...
      if (
        ...
        await tx.insert(ageVerification).values({
          ...
          idReferenceMasked: maskAgeIdReference(
            verificationEvidence.idReference
          ),
          verifiedAt: verificationEvidence.status === 'VERIFIED' ? now : null,
          overrideReason:
            verificationEvidence.status === 'OVERRIDDEN'
              ? verificationEvidence.overrideReason
              : null,
          overrideApprovedBy:
            verificationEvidence.status === 'OVERRIDDEN' ? userId : null,
          overrideApprovedAt:
            verificationEvidence.status === 'OVERRIDDEN' ? now : null,
        });
        await tx
          .insert(auditEvent)
          .values({
      ...
      if (linkedVerification) {
        await tx
          ...
          .where(
            and(
              eq(ageVerification.id, linkedVerification.id),
              sql`${ageVerification.saleId} is null`
            )
          );
        await tx
          ...
          .values({
            ...
            organizationId: orgId,
            userId,
            action: 'age_verification_linked',
            metadata: {
              saleId,
              checkoutId: idempotencyKey,
              verificationId: linkedVerification.id,
            },
          });
      }
      ...
      await tx.insert(saleItem).values(
        saleItems.map((item) => ({
          ...
          totalCost: String(
            costBySaleItem.get(item.saleItemId)?.totalCost ?? 0
          ),
          rewardEligibleAmount: String(
            rewards?.lineEligibility.get(item.productId) ?? 0
          ),
          userId,
          orgId,
        }))
      ...
      const allocatedLots = saleItems.flatMap((item) =>
        (lotsBySaleItem.get(item.saleItemId) ?? []).map((allocation) => ({
          id: generateId(),
          organizationId: orgId,
          saleId,
      ...
      if (allocatedLots.length)
        await tx.insert(saleItemLotAllocation).values(allocatedLots);

      if (resolvedCafe)
        cafeOrderResult = await createCafeOrderForSale(tx, {
          organizationId: orgId,
          branchId: saleBranchId,
          userId,
          saleId,
      ...
      if (
        pharmacyWorkspace &&
        (prescriptionItems.length || restrictedItems.length)
      ) {
        ...
        await tx.insert(pharmacySaleRecord).values({
          ...
          prescriptionReference:
            pharmacyWorkflow?.prescriptionReference || null,
          ...
          verifiedAt: new Date(),
          approvalReason:
            pharmacyWorkflow?.approvalReason || pharmacyWorkflow?.notes || null,
          notes: pharmacyWorkflow?.notes || null,
          approvedBy: restrictedItems.length ? userId : null,
          createdBy: userId,
        });
        const prescribedProductIds = new Set(
          prescriptionItems.map((item) => item.productId)
        );
        const prescriptionLines = saleItems
          .filter((item) => prescribedProductIds.has(item.productId))
          .map((item) => ({
            id: generateId(),
            organizationId: orgId,
        ...
        if (prescriptionLines.length)
          await tx.insert(pharmacyPrescriptionItem).values(prescriptionLines);
        const restrictedProductIds = new Set(
          restrictedItems.map((item) => item.productId)
        );
        const restrictedAuditRows = saleItems
          .filter((item) => restrictedProductIds.has(item.productId))
          .flatMap((item) => {
            ...
            return (
              allocations.length
                ...
                : [
                    {
                      lotId: null,
                      quantity: item.quantity * (item.baseUnitQuantity ?? 1),
                    },
                  ]
            ).map((allocation) => ({
              id: generateId(),
              ...
              approvedBy: userId,
              quantity: String(allocation.quantity),
              customerReference: data.customerId || null,
              reason: pharmacyWorkflow?.notes || 'Restricted medicine sale',
            }));
          });
        if (restrictedAuditRows.length)
          await tx.insert(restrictedItemAudit).values(restrictedAuditRows);
      }
      ...
      if (data.paymentMethod === 'credit') {
        ...
        await tx
          ...
          .values({
            ...
            saleId,
            customerId: data.customerId!,
            amount: financeMoney(calculatedTotal).toFixed(2),
            amountPaid: '0',
            creditedAmount: '0',
        ...
        const invoiceYear = Number(
          new Intl.DateTimeFormat('en', {
            timeZone: orgRecord.timezone || 'UTC',
            year: 'numeric',
          }).format(new Date())
        );
        const [sequence] = await tx
          .insert(invoiceNumberSequence)
          .values({ organizationId: orgId, year: invoiceYear, lastNumber: 1 })
          .onConflictDoUpdate({
            target: [
              invoiceNumberSequence.organizationId,
              invoiceNumberSequence.year,
            ],
            set: {
        ...
        const invoiceDiscount = financeMoney(
          data.discountAmount + (rewards?.externalAmountReduction ?? 0)
        );
        const invoiceState = paymentStatus(
          calculatedTotal,
          0,
        ...
        await tx.insert(invoice).values({
          ...
          businessSnapshot: {
            ...
            kraPin: settings?.taxIdentifier,
            logoUrl: settings?.receiptLogoUrl,
            taxName: settings?.taxName || 'Tax',
          },
          subtotal: financeMoney(serverSubtotal).toFixed(2),
          discountAmount: invoiceDiscount.toFixed(2),
          shippingAmount: financeMoney(shippingAmount).toFixed(2),
          roundingAmount: financeMoney(roundingAmount).toFixed(2),
          taxableAmount: financeMoney(
            settings?.pricesIncludeTax
              ? new Decimal(serverSubtotal).minus(calculatedTax)
              : serverSubtotal
          ).toFixed(2),
          taxRate: String(
            settings?.taxEnabled ? Number(settings.taxRate ?? 0) : 0
          ),
          taxAmount: financeMoney(calculatedTax).toFixed(2),
          total: financeMoney(calculatedTotal).toFixed(2),
          amountPaid: '0',
          creditedAmount: '0',
          balanceDue: invoiceState.balance.toFixed(2),
          fiscalStatus: 'not_submitted',
          idempotencyKey: `pos-credit:${idempotencyKey}`,
          dueDate: data.creditDueDate,
          issuedAt: new Date(),
        ...
        await tx.insert(invoiceItem).values(
          saleItems.map((item, index) => {
            ...
            const taxShare = last
              ? financeMoney(new Decimal(calculatedTax).minus(allocatedTax))
              : financeMoney(
                  serverSubtotal > 0
                    ? new Decimal(calculatedTax).mul(gross).div(serverSubtotal)
                    : 0
                );
            const discountShare = last
              ? financeMoney(invoiceDiscount.minus(allocatedDiscount))
              : financeMoney(
                  serverSubtotal > 0
                    ? invoiceDiscount.mul(gross).div(serverSubtotal)
                    : 0
                );
            allocatedTax = financeMoney(allocatedTax.plus(taxShare));
            allocatedDiscount = financeMoney(
              allocatedDiscount.plus(discountShare)
            );
            const lineTotal = financeMoney(
              settings?.pricesIncludeTax
                ? gross.minus(discountShare)
                : gross.plus(taxShare).minus(discountShare)
            );
            return {
              id: generateId(),
              invoiceId,
              ...
              quantity: item.quantity,
              unitPrice: financeMoney(item.unitPrice).toFixed(2),
              sku: null,
              unit: item.packageName || 'each',
              discountAmount: '0',
              invoiceDiscountShare: discountShare.toFixed(2),
              taxRate: String(
                settings?.taxEnabled ? Number(settings.taxRate ?? 0) : 0
              ),
              taxAmount: taxShare.toFixed(2),
        ...
        await tx
          .update(customerCreditLimit)
          .set({
            currentBalance: financeMoney(
              new Decimal(lockedCreditLimit!.currentBalance).plus(
                calculatedTotal
              )
            ).toFixed(2),
            updatedAt: new Date(),
          })
          .where(eq(customerCreditLimit.id, lockedCreditLimit!.id));
        await tx
          .insert(auditEvent)
          .values({
            id: generateId(),
            ...
            metadata: {
              ...
              branchId: saleBranchId,
              amount: financeMoney(calculatedTotal).toFixed(2),
              dueDate: data.creditDueDate?.toISOString(),
            },
          });
        await tx
          .insert(auditEvent)
      ...
      if (approvedCardAttempt)
        await tx
          .update(cardPaymentAttempt)
          .set({
            status: 'completed',
            saleId,
          ...
          .where(eq(cardPaymentAttempt.id, approvedCardAttempt.id));
    ...
    if (
      ...
      const [duplicate] = await db
        ...
        .from(sale)
        .where(
          and(eq(sale.orgId, orgId), eq(sale.idempotencyKey, idempotencyKey))
        )
        .limit(1);
      if (duplicate) {
        const duplicateItems = await db
          ...
          .from(saleItem)
          .where(
            and(eq(saleItem.saleId, duplicate.id), eq(saleItem.orgId, orgId))
          );
        let etims:
          | Awaited<ReturnType<typeof queueEtimsInvoice>>
          | { status: 'PENDING'; message: string };
        try {
          etims = await queueEtimsInvoice(duplicate.id);
        } catch {
          etims = {
            status: 'PENDING',
            message:
  ...
  return {
    ...
    total: calculatedTotal,
    idempotencyKey,
    items: saleItems.map(({ saleItemId, productId }) => ({
      saleItemId,
      productId,
    })),
    etims,
    cafeOrder: cafeOrderResult,
  };
}
...
export async function syncOfflineSale(input: OfflineSaleSyncInput) {
  ...
  if (
    !authorization.permissions.includes(PermissionEnum.POS_SELL) &&
    !authorization.permissions.includes(PermissionEnum.SALE_CREATE)
  )
    throw new Error('POS sale permission denied');
  const orgId =
    posAuthorization?.organizationId ?? (await getOrgId(userId, 'pos'));
  const [session] = await db
    ...
    .limit(1);
  if (!session?.branchId)
    throw new Error('The original offline-sale shift is unavailable');
  if (
    !authorization.isOrganizationWide &&
    !authorization.branchIds.includes(session.branchId)
  )
    throw new Error('The offline sale belongs to an unauthorized branch');
  if (
    posAuthorization?.terminalId &&
    session.terminalId !== posAuthorization.terminalId
  )
    throw new Error('Synchronize this offline sale from its original register');
  ...
  const [existing] = await db
    ...
    .where(
      and(
        ...
        eq(offlineSaleSync.idempotencyKey, data.queueId)
      )
    )
    .limit(1);
  if (existing && existing.payloadHash !== payloadHash)
    throw new Error(
      'Offline transaction ID was reused with different sale details'
    );

  const syncId = existing?.id ?? generateId();
  await db
    .insert(offlineSaleSync)
    .values({
      id: syncId,
      organizationId: orgId,
      ...
      provisionalReceiptNo: data.provisionalReceiptNo,
      payloadHash,
      status: 'RECEIVED',
      attemptCount: 1,
      offlineCreatedAt: new Date(data.offlineCreatedAt),
      lastAttemptAt: new Date(),
    })
    .onConflictDoUpdate({
      target: [offlineSaleSync.organizationId, offlineSaleSync.idempotencyKey],
      set: {
        status: existing?.status === 'ACCEPTED' ? 'ACCEPTED' : 'RECEIVED',
        attemptCount: sql`${offlineSaleSync.attemptCount} + 1`,
        lastAttemptAt: new Date(),
        errorCode: null,
        errorMessage: null,
        updatedAt: new Date(),
  ...
  try {
    const result = await createSale({
      ...
      offline: {
        ...
        provisionalReceiptNo: data.provisionalReceiptNo,
        createdAt: new Date(data.offlineCreatedAt),
        sessionId: data.sessionId,
      },
    });
    await db
      .update(offlineSaleSync)
      .set({
        status: 'ACCEPTED',
    ...
    await db.insert(auditEvent).values({
      ...
      organizationId: orgId,
      userId,
      action: 'offline_sale_synchronized',
      metadata: {
        syncId,
        saleId: result.saleId,
        ...
        synchronizationDelaySeconds: Math.max(
          0,
          Math.round(
            (Date.now() - new Date(data.offlineCreatedAt).getTime()) / 1000
          )
        ),
      },
    });
    ...
    const message = (
      error instanceof Error
        ? error.message
        : 'Offline sale synchronization failed'
    ).slice(0, 500);
  ...
  const scope = and(
    ...
    search
      ? or(
          ...
          ilike(sql`coalesce(${sale.mpesaRef}, '')`, `%${search}%`),
          sql`exists (select 1 from ${salePayment} where ${salePayment.saleId} = ${sale.id} and ${salePayment.orgId} = ${orgId} and coalesce(${salePayment.reference}, '') ilike ${`%${search}%`})`
        )
      : undefined,
    filters.paymentMethod && filters.paymentMethod !== 'all'
      ? eq(sale.paymentMethod, filters.paymentMethod)
      : undefined,
    filters.status && filters.status !== 'all'
      ? eq(sale.status, filters.status)
      : undefined,
    filters.ageVerification === 'verified'
      ? eq(sale.ageVerified, true)
      : filters.ageVerification === 'not_verified'
        ? eq(sale.ageVerified, false)
        : undefined,
    ...
    endOfDayExclusive(filters.to)
      ? lt(sale.createdAt, endOfDayExclusive(filters.to)!)
      : undefined
  );
...
async function getScopedTotals(
  scope: NonNullable<SalesScope['scope']>,
  orgId: string,
  ...
  const [
    ...
    db
      .select({
        ...
        cogs: sql<string>`coalesce(sum(${saleItem.totalCost}), 0)`,
      })
      .from(saleItem)
      .innerJoin(sale, eq(sale.id, saleItem.saleId))
      .where(and(scope, inArray(sale.status, paidStatuses))),
    db
      .select({
        cost: sql<string>`coalesce(sum(${saleItem.totalCost} * ${salesReturnItem.quantity} / nullif(${saleItem.quantity}, 0)), 0)`,
      })
    ...
    db
      ...
      .where(
        and(
          ...
          filters.from ? gte(expense.createdAt, filters.from) : undefined,
          endOfDayExclusive(filters.to)
            ? lt(expense.createdAt, endOfDayExclusive(filters.to)!)
            : undefined
        )
      ),
  ]);
  const gross = Number(salesTotals?.gross ?? 0),
    refunds = Number(refundTotals?.total ?? 0),
    cogs = Math.max(
      0,
      Number(itemTotals?.cogs ?? 0) - Number(refundedCost?.cost ?? 0)
    ),
    net = gross - refunds,
    grossProfit = net - cogs,
    expensesTotal = Number(expenseTotals?.total ?? 0);
  return {
    ...salesTotals,
    refunds,
    refundCount: Number(refundTotals?.count ?? 0),
    net,
    quantity: Number(itemTotals?.quantity ?? 0),
    cogs,
    grossProfit,
    grossMargin: net ? (grossProfit / net) * 100 : 0,
    expenses: expensesTotal,
    netProfit: grossProfit - expensesTotal,
    average: Number(salesTotals?.transactions ?? 0)
      ? net / Number(salesTotals?.transactions)
      : 0,
  };
}
...
export async function getSalesPageData(filters: SalesPageFilters = {}) {
  const { orgId, authorization, scope, page, pageSize } =
    await getSalesScope(filters);
  const orderColumn =
    filters.sort === 'amount'
      ? sale.total
  ...
  const previousTotals = previous
    ? await getScopedTotals(
        (await getSalesScope(previous)).scope!,
        orgId,
        previous
      )
    : null;
  const compare = (current: number, prior: number) =>
  ...
  const consistentRows = rows.map((row) => ({
    ...row,
    branchName: row.branchName ?? fallbackBranchName,
  }));
  return {
    rows: consistentRows,
    ...
    comparison: previousTotals
      ? {
          net: compare(totals.net, previousTotals.net),
          transactions: compare(
            Number(totals.transactions ?? 0),
            Number(previousTotals.transactions ?? 0)
          ),
          average: compare(totals.average, previousTotals.average),
          grossProfit: compare(totals.grossProfit, previousTotals.grossProfit),
        }
      : null,
  };
}
...
export async function getSalesFilterOptions() {
  ...
  const [customers, cashiers, branches] = await Promise.all([
    db
      .selectDistinct({ id: customer.id, name: customer.name })
      .from(sale)
...
export async function getSalesAnalytics(filters: SalesPageFilters = {}) {
  ...
  const paid = inArray(sale.status, [
    'completed',
    'partially_refunded',
    'refunded',
  ]);
  const [trend, payments, products, cashiers, customers] = await Promise.all([
    db
      .select({
        label: sql<string>`to_char(${sale.createdAt}, 'YYYY-MM-DD')`,
        value: sql<number>`coalesce(sum(${sale.total}), 0)`,
      })
      .from(sale)
      .where(and(scope, paid))
    ...
    db
      .select({
        ...
        quantity: sql<number>`coalesce(sum(${saleItem.quantity}), 0)`,
        value: sql<number>`coalesce(sum(${saleItem.totalPrice}), 0)`,
        profit: sql<number>`coalesce(sum(${saleItem.totalPrice} - ${saleItem.totalCost}), 0)`,
      })
      .from(saleItem)
      .innerJoin(sale, eq(sale.id, saleItem.saleId))
      .where(and(scope, paid))
  ...
  const trendByDate = new Map(
    trend.map((row) => [row.label, Number(row.value)])
  );
...
export async function getSaleWithItems(saleId: string) {
  ...
  const canViewCost =
    authorization.role === RoleEnum.OWNER ||
  ...
  const [
    ...
    db
      ...
      .leftJoin(
        product,
        and(eq(product.id, saleItem.productId), eq(product.orgId, orgId))
      )
      .leftJoin(
        category,
    ...
    db
      .select()
      .from(salePayment)
      .where(and(eq(salePayment.saleId, saleId), eq(salePayment.orgId, orgId)))
      .orderBy(salePayment.createdAt),
    db
      .select({ refund: salesReturn, userName: user.name })
      .from(salesReturn)
      .leftJoin(user, eq(user.id, salesReturn.userId))
      .where(and(eq(salesReturn.saleId, saleId), eq(salesReturn.orgId, orgId)))
      .orderBy(desc(salesReturn.createdAt)),
    saleRecord.record.posSessionId
      ? db
          .select()
          .from(posSession)
          .where(
            and(
              eq(posSession.id, saleRecord.record.posSessionId),
              eq(posSession.orgId, orgId)
            )
          )
          .limit(1)
      : Promise.resolve([]),
    db
      .select({ item: salesReturnItem, returnNo: salesReturn.returnNo })
      .from(salesReturnItem)
      .innerJoin(salesReturn, eq(salesReturn.id, salesReturnItem.returnId))
      .where(
        and(eq(salesReturnItem.orgId, orgId), eq(salesReturn.saleId, saleId))
      ),
    db
      .select({ event: auditEvent, userName: user.name })
      .from(auditEvent)
      .leftJoin(user, eq(user.id, auditEvent.userId))
      .where(
        and(
          eq(auditEvent.organizationId, orgId),
          sql`${auditEvent.metadata}->>'saleId' = ${saleId}`
        )
      )
    ...
    db
      .select({
        ...
        errorMessage: etimsSubmission.errorMessage,
        receiptDetailsEnabled: etimsConfiguration.receiptDetailsEnabled,
      })
      .from(etimsSubmission)
      .innerJoin(
        etimsConfiguration,
        eq(etimsConfiguration.id, etimsSubmission.configurationId)
      )
      .where(
        and(
          eq(etimsSubmission.saleId, saleId),
  ...
  for (const allocation of lotAllocations) {
    allocationsByItem.set(allocation.saleItemId, [
      ...(allocationsByItem.get(allocation.saleItemId) ?? []),
      allocation,
    ]);
  }
  ...
  return {
    ...
    canViewCost,
    payments,
    returns: returns.map(({ refund, ...meta }) => ({ ...refund, ...meta })),
    session: session[0] ?? null,
    refundItems,
    audit,
    etims: fiscalRows[0] ?? null,
    pharmacyRecord: pharmacyRecords[0] ?? null,
  };
}
...
export async function getDashboardStats() {
  ...
  const orgId = await getOrgId(userId);
  const saleScope = authorization.isOrganizationWide
    ? undefined
    : authorization.branchIds.length
      ? inArray(sale.branchId, authorization.branchIds)
      : sql`false`;
  ...
  const [monthSales] = await db
    ...
    .from(sale)
    .where(
      and(eq(sale.orgId, orgId), saleScope, gte(sale.createdAt, monthStart))
    );
  ...
  const weeklyRevenue = await db
    ...
    .from(sale)
    .where(
      and(eq(sale.orgId, orgId), saleScope, gte(sale.createdAt, sevenDaysAgo))
    )
