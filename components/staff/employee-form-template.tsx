...
export function EmployeeFormTemplate({ form, set, open, toggle, branches, shifts, assignableRoles, mode, saving, onSubmit, onCancel }: Props) {
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <div className="flex justify-end gap-3 border-t border-[var(--dashboard-border)] pt-5"><button type="button" onClick={onCancel} disabled={saving} className="inline-flex h-10 items-center rounded-lg border border-[var(--dashboard-border)] bg-[var(--dashboard-surface)] px-4 text-sm font-semibold text-[var(--dashboard-text)] disabled:opacity-60">Cancel</button><button disabled={saving} aria-label={saving ? (mode === 'add' ? 'Adding employee' : 'Updating employee') : undefined} className={`inline-flex h-10 items-center justify-center rounded-lg bg-[var(--dashboard-accent-cta)] text-sm font-semibold text-[var(--dashboard-accent-cta-ink)] hover:bg-[var(--dashboard-accent-cta-hover)] disabled:opacity-60 ${saving ? 'w-10 px-0' : 'px-5'}`}>{saving ? <Loader2 className="h-4 w-4" label={mode === 'add' ? 'Adding employee' : 'Updating employee'} /> : mode === 'add' ? 'Add Employee' : 'Update Employee'}</button></div>
    </form>
  )
}
