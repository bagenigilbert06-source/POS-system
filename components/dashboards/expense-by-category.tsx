...
export function ExpenseByCategoryChart({ data, currency }: ExpenseByCategoryProps) {
  const chartData = data.length > 0 ? data : [{ category: 'No data', count: 0, amount: 1 }]
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[#dfe3ea] bg-white shadow-[0_1px_2px_rgba(16,24,40,.03)]">
      <div className="border-b border-[#edf0f4] px-5 py-4 sm:px-6">
        <h2 className="text-[0.95rem] font-bold text-[#101828]">Expenses by Category</h2>
        <p className="mt-1 text-xs text-[#7b8495]">Breakdown of expenses by category today</p>
      </div>
      <div className="p-4 sm:p-6">
        {chartData[0].category === 'No data' ? (
          <div className="flex h-64 items-center justify-center text-sm text-[#8a94a5]">
            No expense data available for today
          </div>
        ) : (
              ...
              {chartData.map((item, idx) => (
                <div key={item.category} className="flex items-center justify-between text-sm">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-[#8a94a5]">{formatNumber(item.count)} items</p>
                  </div>
                </div>
              ))}
            </div>
