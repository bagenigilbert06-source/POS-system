...
export default async function POSCheckoutPage() {
  ...
  if (!(await getCurrentSession())?.user && !posAuthorization)
    redirect('/sign-in');
  ...
  return (
    <div className="mx-auto max-w-[1480px] space-y-5">
      <DashboardPageHeading
        theme="adaptive"
        icon={CreditCard}
        title={cafeMode ? 'Complete café order' : 'Checkout'}
        description={
          cafeMode
            ? 'Review the order, choose payment, and send prepared items to the right workflow.'
            : 'Review the basket, choose a payment method, and complete the sale securely.'
        }
        action={
          <Link
            href="/dashboard/pos"
            className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-[#d0d5dd] bg-white px-4 text-sm font-semibold text-[#344054] shadow-sm transition-colors hover:bg-[#f9fafb] dark:border-white/10 dark:bg-[#111113] dark:text-[#f5f5f7] dark:hover:bg-[#1d1d1f]"
          >
