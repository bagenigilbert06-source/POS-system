...
export function EtimsConfigurationPanel({ branches, configurations, selectedBranchId, capabilities, providerCredentialsConfigured }: { branches: Branch[]; configurations: SafeConfiguration[]; selectedBranchId?: string; capabilities: EtimsProviderCapabilities; providerCredentialsConfigured: boolean }) {
  ...
  const onboardingRequired = existing?.connectionStatus === 'ONBOARDING_REQUIRED' || existing?.connectionStatus === 'PORTAL_ONBOARDING_REQUIRED'
  ...
  return <section className="app-panel overflow-hidden">
    ...
    <div className="space-y-5 p-5">
      ...
      <div className="rounded-lg border border-amber-300/50 bg-amber-500/[0.06] p-4"><p className="text-xs font-semibold">Integration authorization — action required</p><p className="mt-1 text-xs text-muted-foreground">Complete the OSCU service request in the KRA eTIMS Taxpayer Portal before connecting this branch. This application does not have a certified provider operation to look up the authorization result, so it cannot safely report that KRA is still processing it.</p><p className="mt-2 text-xs font-medium text-amber-700">External OSCU onboarding required</p></div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-t pt-4">{onboardingRequired && <div className="text-xs text-muted-foreground"><p className="font-semibold text-foreground">External OSCU onboarding required</p><p>After completion, a certified status lookup/device-initialization operation must be installed before this branch can be activated.</p></div>}
        {existing && <button type="button" disabled={pending || onboardingRequired || !capabilities.supportsConnectionTest} title={onboardingRequired ? 'Complete onboarding and confirm device activation before testing the connection.' : !capabilities.supportsConnectionTest ? 'This provider does not implement connection testing.' : undefined} onClick={() => startTransition(async () => { const result = await testEtimsConnection(branchId); setTest(result) })} className="inline-flex h-10 items-center gap-2 rounded-lg border px-4 text-sm font-semibold disabled:cursor-not-allowed disabled:text-muted-foreground disabled:opacity-60"><PlugZap className="h-4 w-4" />{onboardingRequired || !capabilities.supportsConnectionTest ? 'Test unavailable' : 'Test connection'}</button>}
