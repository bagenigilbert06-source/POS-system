...
export function CustomerForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (name.length < 2) {
      setError('Enter at least 2 characters for the name.');
      return;
    }
    ...
    try {
      if (customer) {
        ...
        notify.success(`${cafeMode ? 'Guest' : 'Customer'} updated`);
        closeEditor();
      } else {
        ...
        try {
          ...
          notify.success(`${cafeMode ? 'Guest' : 'Customer'} added`);
          closeEditor();
        } catch (err) {
          ...
          <div className="p-5 sm:p-7">
            <div className="mb-6">
              <h2 className="text-lg font-semibold">
                {cafeMode ? 'Guest' : 'Customer'} details
              </h2>
              <p className="mt-1 text-sm text-muted-foreground">
                Name is required. Contact details help your team find people
                during checkout.
              </p>
            </div>
