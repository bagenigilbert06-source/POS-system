...
import { getProductTerminology, type ProductTerminology } from '@/lib/products/terminology'
...
export default async function ReportsPage({ searchParams }: { searchParams?: Promise<Params> }) {
  ...
  const productTerms = getProductTerminology(organization.businessType, organization.businessCategory)
  ...
  return (
    <div className="dashboard-reports mx-auto max-w-[1480px] space-y-4 pb-8">
      <DashboardPageHeading icon={BarChart3} title={pharmacyWorkspace ? 'Pharmacy Reports' : organization.businessCategory === 'liquor_shop' ? 'Liquor Store Reports' : 'Reports'} description={pharmacyWorkspace ? 'Review pharmacy sales, medicine stock, dispensing and operational performance.' : organization.businessCategory === 'liquor_shop' ? 'Review store sales, stock items, payment mix and liquor-store performance.' : 'Review business performance with one consistent date and location scope.'} theme="adaptive" />
      ...
      <nav className="app-panel flex gap-1 overflow-x-auto p-1.5" aria-label="Report sections">
        {sections.map((item) => {
          ...
          return <Link key={item} href={href} aria-current={item === section ? 'page' : undefined} className={`shrink-0 rounded-lg px-3 py-2 text-xs font-semibold capitalize transition-colors ${item === section ? 'bg-[var(--dashboard-accent-soft)] text-[var(--dashboard-accent-strong)]' : 'text-muted-foreground hover:bg-muted/50 hover:text-foreground'}`}>{item === 'products' ? productTerms.title : item}</Link>
        })}
      </nav>

      {pharmacyWorkspace && <section className="grid gap-3 sm:grid-cols-3"><ReportLinkPanel icon={CalendarDays} title="Batch and expiry report" detail="Review medicine lots, expiry windows, quarantined stock, supplier details and return decisions." href="/dashboard/inventory/batches" action="Open batch report" /><ReportLinkPanel icon={ReceiptText} title="Prescription register" detail="Review prescription references, medicines, receipts and dispensing staff." href="/dashboard/pharmacy/prescriptions" action="Open prescription records" /><ReportLinkPanel icon={AlertTriangle} title="Pharmacy safety report" detail="Review expiry value, missing dates, recalls, returns and the controlled-medicine register." href="/dashboard/reports/pharmacy" action="Open safety report" /></section>}

      {section === 'overview' && <Overview report={report} currency={currency} rangeLabel={rangeLabel} terminology={productTerms} />}
      ...
      {section === 'products' && <TopProducts report={report} currency={currency} terminology={productTerms} expanded />}
...
function Overview({ report, currency, rangeLabel, terminology }: { report: ReportsOverview; currency: string; rangeLabel: string; terminology: ProductTerminology }) {
  const metrics = [
...
function TopProducts({ report, currency, terminology, expanded = false }: { report: ReportsOverview; currency: string; terminology: ProductTerminology; expanded?: boolean }) {
  return <section className="report-deferred-section app-panel report-paint-boundary overflow-hidden"><div className="border-b px-4 py-4 sm:px-5"><h2>Top {terminology.pluralLower}</h2><p className="mt-1 text-xs text-muted-foreground">Ranked by recorded sales value in this report period.</p></div>{report.topProducts.length ? <div className="divide-y">{report.topProducts.slice(0, expanded ? 8 : 5).map((item, index) => <div key={item.name} className="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-3 px-4 py-3 sm:grid-cols-[auto_minmax(0,1fr)_auto_auto] sm:px-5"><span className="flex h-8 w-8 items-center justify-center rounded-lg border border-[var(--dashboard-accent-soft-border)] bg-[var(--dashboard-accent-soft)] text-xs font-bold text-[var(--dashboard-accent)]">{index + 1}</span><div className="min-w-0"><p className="truncate text-sm font-semibold">{item.name}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{formatNumber(item.quantity)} units</p></div><p className="text-xs"><span className="text-muted-foreground">Revenue </span><strong className="tabular-nums">{formatCurrency(item.revenue, currency)}</strong></p><p className="text-xs"><span className="text-muted-foreground">Profit </span><strong className="tabular-nums">{item.profit == null ? 'Cost unavailable' : formatCurrency(item.profit, currency)}</strong></p></div>)}</div> : <Empty title={`No ${terminology.singularLower} sales yet`} />}</section>
}

function ProfitSummary({ report, currency, terminology }: { report: ReportsOverview; currency: string; terminology: ProductTerminology }) { const reliable = report.totals.costDataComplete; return <section className="grid gap-3 sm:grid-cols-3"><SummaryCard label="Gross profit" value={reliable ? formatCurrency(report.totals.grossProfit, currency) : 'Cost unavailable'} detail={report.totals.grossMargin == null ? 'Cost data incomplete' : `${report.totals.grossMargin.toFixed(1)}% gross margin`} /><SummaryCard label="Operating expenses" value={formatCurrency(report.totals.expenses, currency)} detail="Recorded expenses" /><SummaryCard label="Net profit" value={reliable ? formatCurrency(report.totals.netProfit, currency) : 'Cost unavailable'} detail={reliable ? 'Gross profit less expenses' : `Complete ${terminology.singularLower} costs to calculate`} /></section> }

function InventoryReport({ report, currency, asOf, terminology }: { report: ReportsOverview; currency: string; asOf: string; terminology: ProductTerminology }) { return <section className="app-panel report-paint-boundary p-5"><div className="flex items-start justify-between"><div><h2>Inventory value as of {dateRangeLabel(asOf, asOf)}</h2><p className="mt-1 text-xs text-muted-foreground">A point-in-time stock valuation, separate from the selected sales performance.</p></div><Package className="h-5 w-5 text-[var(--dashboard-accent)]" /></div><dl className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><SummaryCard label={`${terminology.plural} tracked`} value={formatNumber(report.inventory.products)} detail={`${formatNumber(report.inventory.units)} available units`} /><SummaryCard label="Inventory cost" value={formatCurrency(report.inventory.cost, currency)} detail="Current recorded cost" /><SummaryCard label="Estimated retail value" value={formatCurrency(report.inventory.retailValue, currency)} detail="At current selling prices" /><SummaryCard label="Potential gross margin" value={formatCurrency(Math.max(0, report.inventory.retailValue - report.inventory.cost), currency)} detail="Not recognized revenue" /><SummaryCard label="Low stock" value={formatNumber(report.inventory.lowStock)} detail={`${terminology.plural} at or below minimum`} /><SummaryCard label="Out of stock" value={formatNumber(report.inventory.outOfStock)} detail={`${terminology.plural} with no available units`} /><SummaryCard label="Reorder value" value={formatCurrency(report.inventory.reorderValue, currency)} detail="Cost to restore minimum levels" /></dl></section> }
