...
export class ReceiptPrinterError extends Error {
  constructor(
    public readonly code: ReceiptPrinterErrorCode,
    message: string,
...
export function getReceiptPrinterErrorCopy(error: unknown) {
  const code =
    error instanceof ReceiptPrinterError
      ? error.code
      : 'PRINT_SUBMISSION_FAILED';
  switch (code) {
    case 'SECURITY_NOT_CONFIGURED':
      return {
        title: 'Printer security configuration is incomplete.',
        description: 'Ask an administrator to configure QZ Tray signing.',
      };
    case 'QZ_CONNECTION_TIMEOUT':
      return {
        title: 'Could not connect to QZ Tray.',
        description: 'Check that QZ Tray is running, then try again.',
      };
    case 'QZ_NOT_RUNNING':
      return {
        title: 'QZ Tray is not running.',
        description: 'Start QZ Tray and try again.',
      };
    case 'PRINTER_LOOKUP_TIMEOUT':
      return {
        title: 'Printer lookup timed out.',
        description: 'Check QZ Tray and the Windows printer, then try again.',
      };
    case 'PRINTER_NOT_FOUND':
      return {
        title: 'Configured printer could not be found.',
        description:
          'Check the Windows/QZ printer identifier in device settings.',
      };
    case 'PRINT_SUBMISSION_TIMEOUT':
      return {
        title: 'Printer unavailable.',
        description:
          'The print request timed out. Check the printer and try again.',
      };
    default:
      return {
        title: 'Printer unavailable.',
        description: 'Check the printer connection and try again.',
      };
  }
}

/** A direct print request is possible only when the terminal has a printer name. */
export function hasConfiguredReceiptPrinter(
  settings: Pick<ReceiptPrinterSettings, 'mode' | 'printerName'>
) {
...
export function captureReceiptHtml(element: HTMLElement) {
  ...
  sources.forEach((source, index) => {
    ...
    for (const property of printedStyleProperties)
...
async function qzClient() {
  ...
  if (!securityPromise) {
    securityPromise = withTimeout(
      ...
      new ReceiptPrinterError(
        'SECURITY_NOT_CONFIGURED',
        'QZ security initialization timed out'
      )
    ).catch((error) => {
  ...
  if (isRawTcpPrinter(printerName)) {
    try {
      const response = await fetch('/api/printing/raw-tcp', {
        method: 'HEAD',
        cache: 'no-store',
        headers: { 'x-printer-target': printerName.trim() },
      });
...
export async function directPrintReceipt(
  ...
  if (isRawTcpPrinter(printerName)) {
    ...
    try {
      response = await withTimeout(
        ...
        new ReceiptPrinterError(
          'PRINT_SUBMISSION_TIMEOUT',
          'RAW TCP print submission timed out'
        )
      );
    } catch (error) {
      if (error instanceof ReceiptPrinterError) throw error;
      throw new ReceiptPrinterError(
        'PRINT_SUBMISSION_FAILED',
        'RAW TCP print submission failed',
        { cause: error }
      );
    }
    if (!response.ok) {
      ...
      throw new ReceiptPrinterError(
        response.status === 404
          ? 'PRINTER_NOT_FOUND'
          : 'PRINT_SUBMISSION_FAILED',
        message || 'RAW TCP print submission failed'
      );
    }
  ...
  try {
    ...
    throw new ReceiptPrinterError(
      'PRINTER_NOT_FOUND',
      'Configured printer was not found',
      { cause: error }
    );
  }
  ...
  if (
    !names.some(
      (name) =>
        name.localeCompare(printerName, undefined, {
          sensitivity: 'accent',
        }) === 0
    )
  ) {
  ...
  const config = qz.configs.create(printerName, {
    units: 'mm',
    size: { width: settings.paperWidth },
    margins: 0,
    colorType: 'grayscale',
    copies: Math.max(1, Math.min(3, settings.copies)),
    scaleContent: true,
...
/** Sends only the standard ESC/POS drawer pulse. Authorization is performed before this is called. */
export async function openQzCashDrawer(
  printerName: string
): Promise<ReceiptPrintResult> {
  const trustedName = printerName.trim();
  if (
    !trustedName ||
    isRawTcpPrinter(trustedName) ||
    /^microsoft print to pdf$/i.test(trustedName)
  )
    throw new ReceiptPrinterError(
      'PRINTER_NOT_FOUND',
      'A compatible drawer printer is not configured'
    );
  const qz = await qzClient();
  const found = await withTimeout(
    qz.printers.find(trustedName),
    QZ_PRINTER_LOOKUP_TIMEOUT_MS,
    new ReceiptPrinterError(
      'PRINTER_LOOKUP_TIMEOUT',
      'Printer lookup timed out'
  ...
  if (
    !names.some(
      (name) =>
        name.localeCompare(trustedName, undefined, {
          sensitivity: 'accent',
        }) === 0
    )
  )
  ...
  try {
    await withTimeout(
      qz.print(qz.configs.create(trustedName), [
        {
          type: 'raw',
          format: 'command',
          flavor: 'plain',
...
export function browserPrintReceipt(
  receiptHtml: string,
  paperWidth: ReceiptPaperWidth
): ReceiptPrintResult {
  ...
  if (!printWindow || !printDocument) {
    removeFrame();
    throw new Error('Could not open browser printing');
  }
  ...
  printDocument.open();
  printDocument.write(
    thermalDocument(receiptHtml, paperWidth).replace(
      '<head>',
      `<head>${assets}`
    )
  );
  ...
  printWindow.addEventListener(
    'afterprint',
    () => window.setTimeout(removeFrame, 250),
    { once: true }
  );
  window.setTimeout(() => {
    printWindow.focus();
