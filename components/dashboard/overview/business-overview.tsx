...
const PANEL =
  'rounded-xl border border-[var(--dashboard-border)] bg-[var(--dashboard-surface)] shadow-dark-sm';
...
function yesterdayTrend(current: number, previous: number): MetricTrend {
  ...
  if (Math.abs(change) < 0.05)
    return { direction: 'neutral', text: 'No change vs yesterday' };
  return {
...
export function BusinessOverview({
  ...
  const saleHref = workspaceConfig.enabledModules.includes('pos')
    ? '/dashboard/pos'
    : '/dashboard/sales';
  const availableActions = [
    ...((workspaceConfig.enabledModules.includes('pos') ||
      workspaceConfig.enabledModules.includes('sales')) &&
    permissions.includes(PermissionEnum.POS_VIEW)
      ? [
          {
            id: 'primary',
            label: experience.actionLabels.primary,
            href: saleHref,
            icon: ShoppingBag,
            primary: true,
      ...
      : []),
    ...(hasProducts && permissions.includes(PermissionEnum.PRODUCT_VIEW)
      ? [
          {
            id: 'products',
            label: experience.actionLabels.products,
            href: '/dashboard/products',
            icon: Package,
            primary: false,
            description: `Manage ${productTerms.pluralLower}`,
          },
        ]
      : []),
    ...(hasInventory && permissions.includes(PermissionEnum.INVENTORY_VIEW)
      ? [
          {
            id: 'inventory',
            label: experience.actionLabels.inventory,
      ...
      : []),
    ...(hasCustomers && permissions.includes(PermissionEnum.CUSTOMER_VIEW)
      ? [
          {
            id: 'customers',
            label: 'Customers',
      ...
      : []),
    ...(workspaceConfig.enabledModules.includes('reports') &&
    permissions.includes(PermissionEnum.REPORT_VIEW)
      ? [
          {
            id: 'reports',
            label: 'Reports',
  ...
  const metrics = [
    ...
    },
    {
      ...
      trend:
        profitAvailable && overview.previousDay.grossProfit !== null
          ? yesterdayTrend(todayProfit, overview.previousDay.grossProfit)
          : undefined,
      primaryMeta: profitAvailable
        ? undefined
  ...
  return (
    <div className="dashboard-overview mx-auto w-full max-w-[1440px] space-y-4 pb-8">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            ...
            <p className={cn('mt-2 max-w-lg text-sm leading-6', MUTED)}>
              {role === RoleEnum.MANAGER
                ? `Today's results and alerts for your assigned ${overview.records.branches === 1 ? 'branch' : 'branches'}.`
                : `Today's business summary for ${organizationName}.`}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 lg:shrink-0">
            ...
            {secondaryActions.length > 0 && (
              <div
                className={cn(
                  'flex items-center divide-x overflow-hidden rounded-lg border',
                  DIVIDER,
                  'divide-[var(--dashboard-border)] bg-[var(--dashboard-surface)]'
                )}
              >
                {secondaryActions.map((action) => {
                  ...
                  return (
                    <Link
                      ...
                      href={action.href}
                      className={cn(
                        'inline-flex h-10 items-center gap-2 px-3.5 text-sm font-medium outline-none transition-colors hover:bg-[var(--dashboard-surface-subtle)] focus-visible:relative focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--dashboard-accent-cta)]',
                        TEXT
                      )}
                    >
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {metrics.map((metric) => (
            <MetricCard
              ...
              href={metric.href}
              trend={metric.trend}
              primaryMeta={metric.primaryMeta}
      ...
      <DashboardInsightCharts
        ...
        stock={{
          healthy: overview.records.products - overview.records.lowStock,
          low: overview.records.lowStock - overview.records.outOfStock,
      ...
      {isPharmacyBusiness(
        ...
        <section
          aria-label="Pharmacy stock controls"
        ...
        >
          ...
          >
            <div>
              <h2 className={cn('text-sm font-bold', TEXT)}>
                Batch and expiry attention
              </h2>
              <p className={cn('mt-0.5 text-xs', MUTED)}>
                Actionable medicine stock risks from the inventory ledger.
              </p>
            </div>
            <Link
              href="/dashboard/inventory/batches"
              className={cn('text-xs font-semibold hover:underline', TEXT)}
            >
              Review batches
            </Link>
          </div>
          <div className="grid divide-y sm:grid-cols-3 sm:divide-x sm:divide-y-0">
            <div className="p-4">
              <p className={cn('text-xs', MUTED)}>Expiring within 90 days</p>
              <p className={cn('mt-1 text-xl font-bold tabular-nums', TEXT)}>
                {overview.pharmacyInventory.expiringSoon}
              </p>
            </div>
            <div className="p-4">
              <p className={cn('text-xs', MUTED)}>Expired batches</p>
              <p className="mt-1 text-xl font-bold tabular-nums text-red-600">
                {overview.pharmacyInventory.expired}
              </p>
            </div>
            <div className="p-4">
              <p className={cn('text-xs', MUTED)}>Value at risk</p>
              <p className={cn('mt-1 text-xl font-bold tabular-nums', TEXT)}>
                {formatCurrency(
                  overview.pharmacyInventory.valueAtRisk,
                  currency
                )}
              </p>
            </div>
...
function PaymentBadge({ method }: { method: string }) {
  ...
  if (normalized.includes('mpesa') || normalized.includes('m pesa')) {
    return (
      <span
        className="inline-flex h-7 items-center gap-1.5 rounded-lg border border-[var(--dashboard-success-soft-border)] bg-[var(--dashboard-success-soft)] px-2.5 text-[0.68rem] font-semibold text-[var(--dashboard-success)]"
        aria-label="Paid with M-Pesa"
      >
  ...
  if (normalized.includes('card')) {
    return (
      <span
        className="inline-flex h-7 items-center gap-1.5 rounded-lg border border-[var(--dashboard-border)] bg-[var(--dashboard-surface-subtle)] px-2.5 text-[0.68rem] font-semibold text-[var(--dashboard-text)]"
        aria-label="Paid by card"
      >
        <CreditCard
          className="h-3 w-3 text-[var(--dashboard-muted)]"
          aria-hidden="true"
        />
        Card
      </span>
  ...
  if (normalized.includes('bank')) {
    return (
      <span className="inline-flex h-7 items-center gap-1.5 rounded-lg border border-[var(--dashboard-border)] bg-[var(--dashboard-surface-subtle)] px-2.5 text-[0.68rem] font-semibold text-[var(--dashboard-text)]">
        <Landmark
          className="h-3 w-3 text-[var(--dashboard-muted)]"
          aria-hidden="true"
        />
  ...
  return (
    <span className="inline-flex h-7 items-center gap-1.5 rounded-lg border border-[var(--dashboard-accent-soft-border)] bg-[var(--dashboard-accent-soft)] px-2.5 text-[0.68rem] font-semibold text-[var(--dashboard-accent)]">
...
function TodayRegisterCard({
  ...
  return (
    <div className={cn(embedded ? '' : cn(PANEL, 'overflow-hidden'))}>
      ...
      >
        <div>
          <h2 className={cn('text-[0.95rem] font-bold', TEXT)}>
            Register summary
          </h2>
          <p className={cn('mt-0.5 text-xs', MUTED)}>
            Today&apos;s checkout activity.
          </p>
        </div>
      </div>
      ...
      <dl className={cn('grid grid-cols-2 divide-x border-b', DIVIDER)}>
        <div className="px-5 py-3">
          <dt
            className={cn(
              'text-[0.66rem] font-medium uppercase tracking-[0.08em]',
              MUTED
            )}
          >
            Sales
          </dt>
          <dd className={cn('mt-1 text-lg font-bold tabular-nums', TEXT)}>
      ...
      <div className="space-y-1.5 px-4 py-3 text-sm">
        <div className="flex items-center justify-between gap-3">
          ...
          <strong className={cn('tabular-nums', TEXT)}>
            {formatCurrency(averageSale, currency)}
          </strong>
        </div>
        <div className="flex items-center justify-between gap-3">
      ...
      <div
        className={cn(
          'grid grid-cols-2 gap-2 border-t bg-[var(--dashboard-surface-subtle)] p-2',
          DIVIDER
        )}
      >
        <Link
          href={saleHref}
          className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-[var(--dashboard-accent-cta)] px-3 text-xs font-bold text-[var(--dashboard-accent-cta-ink)] transition-colors hover:bg-[var(--dashboard-accent-cta-hover)]"
        >
        ...
        <Link
          href="/dashboard/sales"
          className={cn(
            'inline-flex h-10 items-center justify-center gap-2 rounded-lg border bg-[var(--dashboard-surface)] px-3 text-xs font-semibold transition-colors hover:bg-[var(--dashboard-surface-subtle)]',
            DIVIDER,
            TEXT
          )}
        >
...
function MiniRecord({
  ...
  return (
    <Link
      href={href}
      className={cn(
        'border-r px-3 py-3.5 text-center transition-colors last:border-r-0 hover:bg-[var(--dashboard-surface-subtle)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--dashboard-accent-cta)]',
        DIVIDER
      )}
    >
      <span className={cn('block text-sm font-bold tabular-nums', TEXT)}>
        {value}
      </span>
...
function EmptyState({
  ...
  return (
    <div className="flex min-h-44 flex-col items-center justify-center px-6 py-8 text-center">
      <span
        className={cn(
          'flex h-11 w-11 items-center justify-center rounded-lg bg-[var(--dashboard-surface-subtle)]',
          MUTED
        )}
      >
      ...
      <Link
        href={href}
        className="mt-4 text-xs font-semibold text-[var(--dashboard-accent)] hover:opacity-80"
      >
        {action}
      </Link>
    </div>
