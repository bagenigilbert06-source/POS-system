...
export function offlineWorkspaceStorageKey(
  organizationId: string,
  key: 'cart' | 'checkout-id' | 'mpesa'
) {
...
export function bindPosConnectivityEvents(
  target: Window,
...
export function checkoutAlreadyQueued(
...
export function offlineAmountConflicts(
  cached: number,
  ...
  return (
    ...
    Math.abs(cached - authoritative) > tolerance
  );
}
...
export function isConnectivityFailure(error: unknown) {
  ...
  return /failed to fetch|network(?: request)? failed|networkerror|load failed|connection (?:was )?(?:lost|closed|refused)|fetch failed/i.test(
    message
  );
}
