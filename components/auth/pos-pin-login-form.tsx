...
import { getPosTerminalStaff, unlockPosWithStaffPin } from '@/app/actions/pos-pin'
...
export function PosPinLoginForm() {
  ...
  const submit = () => {
    ...
    start(async () => {
      setError('')
      const result = await unlockPosWithStaffPin(selectedStaffId, pin)
      if (!result.success) {
        setPin('')
  ...
  const canSubmit = Boolean(selectedStaffId && selectedStaff?.pinSet && pin.length === 6)
  ...
  return <div className="w-full space-y-5">
      ...
      {loadingStaff ? (
        ...
        <div className="relative">
          ...
          <select id="pos-staff" value={selectedStaffId} onChange={(event) => { setSelectedStaffId(event.target.value); setPin(''); setError('') }} disabled={pending || staff.length === 0} className="h-12 w-full appearance-none rounded-lg border border-zinc-300 bg-white pl-10 pr-4 text-sm font-semibold text-zinc-900 outline-none transition-colors focus:border-[#d4aa00] focus:ring-2 focus:ring-[#ffda32]/30 disabled:cursor-not-allowed disabled:bg-zinc-100">
            ...
            {staff.map((member) => <option key={member.id} value={member.id}>{member.name} · {member.role}{member.pinSet ? '' : ' (PIN not set)'}</option>)}
          </select>
        </div>
      )}
    ...
    <p className="text-center text-xs leading-5 text-zinc-500">Choose your staff profile, then use your six-digit POS PIN.</p>
  </div>
}
