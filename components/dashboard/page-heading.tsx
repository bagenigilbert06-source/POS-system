...
export function DashboardPageHeading({ icon: Icon, eyebrow = 'Pesaby workspace', title, description, action, theme = 'light' }: { icon: ElementType; eyebrow?: string; title: string; description: string; action?: ReactNode; theme?: 'light' | 'dark' | 'adaptive' }) {
  ...
  return (
    <header className={dark
      ...
      : adaptive
      ? 'flex flex-col gap-4 rounded-2xl border border-[#eadfbd] bg-gradient-to-r from-[#fffdf7] via-[#fffdf7] to-[#fff8d6] p-6 shadow-[0_2px_8px_rgba(16,24,40,.04)] dark:border-[rgba(255,214,10,0.2)] dark:from-[#0f0f0f] dark:via-[#151515] dark:to-[#211d0d] dark:shadow-[0_2px_8px_rgba(0,0,0,.18)] sm:flex-row sm:items-center sm:justify-between'
