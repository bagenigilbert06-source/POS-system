...
type SafeConfiguration = { branchId: string; environment: 'sandbox' | 'production'; integrationMethod: 'OSCU' | 'VSCU'; businessKraPin: string; externalBranchId: string; vatRegistered: boolean; providerName: string; deviceId: string; connectionStatus: string; lastConnectionTestAt: Date | null }
...
export function EtimsConfigurationPanel({ branches, configurations, selectedBranchId, capabilities }: { branches: Branch[]; configurations: SafeConfiguration[]; selectedBranchId?: string; capabilities: EtimsProviderCapabilities }) {
  ...
  const ready = Boolean(branchId && pin.trim() && externalBranchId.trim())
  ...
  return <section className="app-panel overflow-hidden">
    ...
    <div className="space-y-5 p-5">
      <div className="grid gap-2 sm:grid-cols-4">{['Business identity','eTIMS branch','Device / connection','Verify & activate'].map((label, index) => <div key={label} className="rounded-lg border bg-muted/20 px-3 py-2 text-xs font-semibold"><span className="mr-2 text-primary">{index + 1}</span>{label}</div>)}</div>
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <label className="text-xs font-semibold text-muted-foreground">eTIMS branch (after authorization)<input aria-label="eTIMS Branch ID" value={externalBranchId} onChange={(e) => setExternalBranchId(e.target.value)} className={input} /></label>
      ...
      <div className="flex flex-wrap justify-end gap-2 border-t pt-4">
        {existing && <button type="button" disabled={pending} onClick={() => startTransition(async () => { const result = await testEtimsConnection(branchId); setTest(result) })} className="inline-flex h-10 items-center gap-2 rounded-lg border px-4 text-sm font-semibold"><PlugZap className="h-4 w-4" />Test connection</button>}
        <button type="button" disabled={pending || !ready} onClick={save} className="inline-flex h-10 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground disabled:opacity-50">{pending ? <Loader2 className="h-4 w-4 animate-spin" /> : existing ? <Save className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />}{existing ? 'Save changes' : 'Verify & activate'}</button>
      </div>
    </div>
  </section>
}
