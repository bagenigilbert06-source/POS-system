...
export function POSTerminal({
  ...
  const processCheckout = async (
    ...
    try {
      ...
      // Feedback is optional and must never delay a completed sale. Add its
      // QR token to the already-rendered receipt only if the follow-up returns.
      void createSaleFeedbackInvitation(saleId)
        .then((invitation) => {
          if (!invitation) return;
          setReceipt((current) =>
            current?.saleId === saleId
              ? { ...current, feedbackToken: invitation.token }
              : current
          );
        })
        .catch(() => undefined);
      ...
      >
        <div className="w-full max-w-[1020px]">
          ...
          <div className="grid overflow-hidden rounded-[10px] border border-[#dfe3ea] bg-white shadow-[0_4px_16px_rgba(16,24,40,.06)] dark:border-white/10 dark:bg-[var(--dashboard-surface)] lg:grid-cols-[minmax(0,1fr)_360px]">
            <div className="flex min-h-[500px] items-center justify-center bg-[#f3f5f7] p-5 shadow-[inset_-1px_0_0_rgba(16,24,40,.04)] dark:bg-[var(--dashboard-surface-subtle)] sm:p-8">
