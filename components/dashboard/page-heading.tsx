  ...
  return (
    <header className={dark
      ...
      <div className="flex min-w-0 items-center gap-4">
        ...
        <div className="min-w-0"><p className={dark || adaptive ? 'text-[11px] font-bold uppercase tracking-[0.14em] text-[var(--dashboard-accent)]' : 'text-[11px] font-bold uppercase tracking-[0.14em] text-[#a47700]'}>{eyebrow}</p><h1 className={dark || adaptive ? 'mt-1 text-xl font-bold tracking-tight text-[var(--dashboard-text)] sm:text-2xl' : 'mt-1 text-xl font-bold tracking-tight text-[#101828] sm:text-2xl'}>{title}</h1><p className={dark || adaptive ? 'mt-1 text-sm text-[var(--dashboard-muted)]' : 'mt-1 text-sm text-[#667085]'}>{description}</p></div>
      </div>
      {action}
    </header>
  )
