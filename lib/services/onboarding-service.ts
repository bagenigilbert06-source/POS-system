...
function migrateLegacyDraft(org: typeof organization.$inferSelect): OnboardingDraft {
  ...
  return {
    ...
    phone: org.phone ?? '',
    businessSize: (org.businessSize as OnboardingDraft['businessSize']) ?? 'small',
  ...
  static async complete(userId: string, emailVerified: boolean, expectedRevision: number) {
    return db.transaction(async (tx) => {
      ...
      const settingsValues = {
        ...
        receiptLogoUrl: data.receiptLogoUrl || null,
        ...
        receiptShowCustomer: data.receiptShowCustomer,
        receiptShowPayment: data.receiptShowPayment,
        receiptShowQrCode: data.receiptShowQrCode,
        receiptShowItemSku: data.receiptShowItemSku,
