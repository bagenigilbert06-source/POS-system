...
export async function POST(request: Request) {
  ...
  try { organizationId = (await requirePermission(PermissionEnum.SETTINGS_EDIT)).organizationId } catch (error) { return NextResponse.json({ error: error instanceof AuthorizationError ? error.message : 'Unauthorized' }, { status: 403 }) }
