...
export function ReceiptTemplate({
  ...
  if (layout === 'thermal') {
    ...
    return <div className="receipt-paper receipt-thermal mx-auto w-full max-w-full bg-white px-[3mm] py-[4mm] font-['Courier_New',Courier,monospace] text-[10px] leading-[1.4] text-black print:w-full print:max-w-none print:px-[2mm] print:py-[3mm]">
      ...
      <div className="mb-2 grid grid-cols-[minmax(0,1fr)_24px_74px] gap-1 border-b border-dashed border-black pb-1 text-[9px] font-bold uppercase"><span>Description</span><span className="text-center">Qty</span><span className="text-right">Amount</span></div>
      <div>{sale.items.map((item) => <div key={item.id} className="grid break-inside-avoid grid-cols-[minmax(0,1fr)_24px_74px] gap-1 border-b border-dotted border-zinc-400 py-1.5"><span className="min-w-0 break-words">{item.productName}{showItemSku && <span className="mt-0.5 block text-[8px] font-normal leading-3">SKU: {item.productId.slice(0, 8).toUpperCase()}</span>}</span><span className="text-center tabular-nums">{item.quantity}</span><span className="whitespace-nowrap text-right tabular-nums">{formatCurrency(parseFloat(item.totalPrice.toString()))}</span></div>)}</div>
      <div className="my-[3mm] border-y border-dashed border-black py-[2mm] tabular-nums"><div className="flex justify-between"><span>Subtotal</span><span>{formatCurrency(subtotal)}</span></div>{taxAmount > 0 && <div className="flex justify-between"><span>{taxName}</span><span>{formatCurrency(taxAmount)}</span></div>}{discountAmount > 0 && <div className="flex justify-between"><span>Discount</span><span>-{formatCurrency(discountAmount)}</span></div>}{roundingAmount !== 0 && <div className="flex justify-between"><span>Rounding</span><span>{roundingAmount > 0 ? '+' : '-'}{formatCurrency(Math.abs(roundingAmount))}</span></div>}<div className="mt-1 flex justify-between border-t border-dashed border-black pt-1 text-[13px] font-bold"><span>TOTAL</span><span>{formatCurrency(total)}</span></div></div>
