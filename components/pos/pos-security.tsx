...
import { LockKeyhole, MonitorSmartphone, ShieldCheck, X } from 'lucide-react'
import { getOwnPosPinStatus, lockPos, registerCurrentPosTerminal, setOwnPosPin } from '@/app/actions/pos-pin'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'

export function PosSecurity({ branchId, initialPinSet = false, terminalRegistered = false }: { branchId: string; initialPinSet?: boolean; terminalRegistered?: boolean }) {
  ...
  return <>
    ...
    {registerOpen && <div className="fixed inset-0 z-[70] grid place-items-center bg-slate-950/65 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="pos-terminal-title"><div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl"><h2 id="pos-terminal-title" className="text-lg font-semibold text-slate-900">Register this POS device</h2><p className="mt-1 text-sm text-slate-600">An administrator can bind this browser to a physical terminal for this branch.</p><Input autoFocus className="mt-5" value={terminalName} onChange={event => setTerminalName(event.target.value)} maxLength={80} placeholder="e.g. Terminal 1" />{error && <p className="mt-3 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">{error}</p>}<div className="mt-5 flex justify-end gap-2"><Button variant="outline" onClick={() => setRegisterOpen(false)}>Cancel</Button><Button disabled={pending || terminalName.trim().length < 2} onClick={() => run(async () => { await registerCurrentPosTerminal(branchId, terminalName); setRegisterOpen(false); router.refresh() })}>Register device</Button></div></div></div>}
