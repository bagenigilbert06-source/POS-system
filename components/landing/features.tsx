    ...
    <section id="features" className="section-padding-premium bg-secondary">
      <div className="container-wide">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12 sm:mb-16 md:mb-20 px-4">
          <p className="section-eyebrow mb-2 sm:mb-3">Platform Features</p>
        ...
        {/* 3 featured cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5 mb-4 sm:mb-5 px-2 sm:px-0">
          {featured.map((f) => {
            const Icon = f.icon
            return (
              <div
                key={f.title}
                className="fluent-card-hover flex flex-col gap-4 sm:gap-5 p-5 sm:p-6 md:p-7"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className={`h-10 sm:h-11 w-10 sm:w-11 rounded-xl flex items-center justify-center flex-shrink-0 ${f.iconClass}`}>
                ...
                <div>
                  <h3 className="text-sm sm:text-base font-bold text-foreground mb-1 sm:mb-2">{f.title}</h3>
        ...
        <div className="grid overflow-hidden rounded-2xl border border-border bg-border shadow-sm grid-cols-1 sm:grid-cols-2 md:grid-cols-3 px-2 sm:px-0">
          {secondary.map((f) => {
            const Icon = f.icon
            return (
              <div
                key={f.title}
                className="group flex items-start gap-3 sm:gap-4 bg-card p-4 sm:p-5 md:p-6 transition-colors duration-150 hover:bg-card/70"
              >
                ...
                <div>
                  <h3 className="text-xs sm:text-sm font-semibold text-foreground mb-0.5 sm:mb-1">{f.title}</h3>
