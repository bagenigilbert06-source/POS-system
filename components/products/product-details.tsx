...
export function ProductDetails({ overview }: { overview: ProductOverview }) {
  ...
  const margin = buying > 0 ? (profit / buying) * 100 : 0
  ...
  return <div className="mx-auto max-w-[1100px] space-y-5">
    ...
    <section className="overflow-hidden rounded-xl border border-[#dfe3ea] bg-white shadow-sm"><div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[300px_1fr]">
      ...
      <div><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-[#9a6900]">{categoryName ?? 'Product'}</p><h1 className="mt-1 text-2xl font-bold text-[#101828]">{product.name}</h1><p className="mt-2 text-sm text-muted-foreground">{product.sku ? `SKU ${product.sku}` : 'No SKU'}{product.barcode ? ` · Barcode ${product.barcode}` : ''}</p></div><span className="rounded-full bg-[#edf7ef] px-3 py-1 text-xs font-semibold text-[#28743c]">{status}</span></div><div className="mt-7 grid gap-4 sm:grid-cols-2"><Metric label="Selling price" value={formatCurrency(selling)} /><Metric label="Cost price" value={formatCurrency(buying)} /><Metric label="Profit per unit" value={formatCurrency(profit)} /><Metric label="Margin" value={`${margin.toFixed(1)}%`} /></div><div className="mt-6 flex flex-wrap gap-2"><Link href={`/dashboard/products/${product.id}?edit=true`} className="inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground"><Edit3 className="h-4 w-4" /> Edit product</Link></div></div>
    </div></section>
