...
export async function POST(request: Request) {
  try {
    ...
    if (matches[0] && matches[0].stock <= 0) return NextResponse.json({ error: `${matches[0].name} is out of stock.` }, { status: 409 })
    ...
    return NextResponse.json(matches[0]
      ? { accepted: true, registered: true, product: { name: matches[0].name } }
      : { accepted: true, registered: false, barcode, message: 'New barcode sent to the POS for product registration.' },
