      ...
      {isEditing ? (
        <div className="mx-auto max-w-[1180px] space-y-5 rounded-2xl border border-[var(--dashboard-border)] bg-[var(--dashboard-surface)] p-5 shadow-[0_18px_48px_rgba(0,0,0,.16)] sm:p-6">
          ...
          {(!section || section === 'operating') && <div className="space-y-4 border-b pb-6">
            ...
            <div><label className="text-sm font-medium">Enabled payment methods</label><div className="mt-2 grid gap-2 sm:grid-cols-3 lg:grid-cols-6">{(['cash', 'mpesa', 'airtel_money', 'card', 'bank_transfer', 'credit'] as const).map((method) => { const checked = formData.paymentMethods.includes(method); return <label key={method} className="flex cursor-pointer items-center gap-3 rounded-lg border p-3 capitalize"><input type="checkbox" checked={checked} onChange={(event) => { const methods = event.target.checked ? [...formData.paymentMethods, method] : formData.paymentMethods.filter((item) => item !== method); const defaultPaymentMethod = methods.includes(formData.defaultPaymentMethod) ? formData.defaultPaymentMethod : methods[0] || ''; setFormData({ ...formData, paymentMethods: methods, defaultPaymentMethod }) }} className="h-4 w-4 accent-[#e42527]" />{method === 'mpesa' ? 'M-Pesa' : method === 'airtel_money' ? 'Airtel Money' : method === 'bank_transfer' ? 'Bank transfer' : method === 'credit' ? 'Customer credit' : method}</label> })}</div></div>
