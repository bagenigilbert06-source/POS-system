...
export default async function PharmacyAlertsPage() {
  ...
  const alerts = [
    ...
    ...lowStock.map((item) => ({ severity: Number(item.stock) <= 0 ? 'critical' : 'warning', title: `${Number(item.stock) <= 0 ? 'Out of stock' : 'Low stock'}: ${item.name}`, detail: `${item.stock} units available; reorder level ${item.minStock}.`, href: '/dashboard/purchases', action: 'Create purchase' })),
