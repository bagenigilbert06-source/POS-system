...
export function StartRecallForm({ lotId }: { lotId: string }) {
  ...
  return <form className="grid min-w-64 gap-2" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); startTransition(async () => { try { await initiateMedicineRecall({ lotId, reference: String(form.get('reference')), reason: String(form.get('reason')) }); toast.success('Batch recalled and blocked from sale'); setOpen(false) } catch (error) { toast.error(error instanceof Error ? error.message : 'Recall failed') } }) }}>
...
export function ResolveRecallForm({ recallId }: { recallId: string }) {
  const [pending, startTransition] = useTransition()
  return <form className="flex flex-wrap items-center gap-2" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); startTransition(async () => { try { await resolveMedicineRecall({ recallId, resolution: String(form.get('resolution')) as 'release' | 'dispose', notes: String(form.get('notes')) }); toast.success('Recall resolved') } catch (error) { toast.error(error instanceof Error ? error.message : 'Resolution failed') } }) }}>
