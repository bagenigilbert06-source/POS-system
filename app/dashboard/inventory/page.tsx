...
export default async function InventoryPage() {
  ...
  const totalValue = activeProducts.reduce(
    (sum, item) => sum + Number(item.buyingPrice) * item.stock,
    0
  );
  ...
  const pendingCounts = control.adjustments.filter(
  ...
  const metrics = [
    ...
    },
    {
      ...
      detail: `${activeProducts.reduce((sum, item) => sum + item.stock, 0)} total units tracked`,
      icon: PackageCheck,
      tone: 'default',
    },
    {
      ...
      detail: `${lowStock.length} low · ${outOfStock.length} out of stock`,
      icon: AlertTriangle,
      tone: lowStock.length + outOfStock.length ? 'warning' : 'success',
    },
    {
      label: 'Pending counts',
      value: String(pendingCounts.length),
      detail: pendingCounts.length
        ? 'Awaiting review and approval'
        : 'All physical counts reviewed',
      icon: ClipboardCheck,
      tone: pendingCounts.length ? 'warning' : 'success',
    },
  ] as const;
