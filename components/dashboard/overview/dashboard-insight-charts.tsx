...
// Keep charts within the dashboard's warm, understated palette. Red is reserved
// for genuine errors/stock-outs rather than being used as a general data color.
const COLORS = ['#d6a800', '#1f9d68', '#3d6fbd', '#7b8495', '#8b6bb1']
...
function ChartCard({ title, description, href, children }: { title: string; description: string; href?: string; children: React.ReactNode }) {
  return <article className="overflow-hidden rounded-xl border border-[var(--dashboard-border)] bg-[var(--dashboard-surface)] shadow-[0_1px_2px_rgba(16,24,40,.03)]">
    <div className="flex items-center justify-between gap-4 border-b border-[var(--dashboard-border)] px-5 py-3">
      ...
      {href && <Link href={href} className="inline-flex items-center gap-1 text-xs font-semibold text-[var(--dashboard-text)] hover:text-[#a47700]">View all <ArrowRight className="h-3.5 w-3.5" /></Link>}
    </div>
    {children}
  </article>
}
...
export function DashboardInsightCharts({ currency, paymentMix, topProducts, stock, productLabel = 'products' }: DashboardInsightChartsProps) {
  ...
  const stockData = [
    ...
    { name: 'Out', value: stock.out, color: '#d36b5f' },
