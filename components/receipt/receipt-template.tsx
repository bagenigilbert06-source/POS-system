...
export function ReceiptTemplate({
  ...
  if (layout === 'thermal') {
    return <div style={{ fontFamily: '"Roboto Mono", "Courier New", monospace' }} className="receipt-paper receipt-thermal receipt-java-style mx-auto w-full max-w-full bg-white px-[3mm] py-[3mm] text-[10pt] font-normal leading-[1.25] text-black print:w-full print:max-w-none print:px-[2mm] print:py-[2mm]" {...{ [THERMAL_RECEIPT_DATA_ATTRIBUTE]: thermalReceiptData }}>
      <div className="text-center">{logoUrl ? <span className="mb-2 flex h-[20mm] items-center justify-center"><Image src={logoUrl} alt={`${businessName} logo`} width={240} height={160} unoptimized className="max-h-[20mm] w-auto max-w-[30mm] object-contain grayscale" /></span> : null}<h1 className="text-[16pt] font-bold uppercase leading-tight">{businessName}</h1>{branchName && <p className="text-[9pt]">{branchName}</p>}{showAddress && businessAddress && <p className="whitespace-pre-line text-[9pt]">{businessAddress}</p>}{showPhone && businessPhone && <p className="text-[9pt]">Tel: {businessPhone}</p>}{kraPin && <p className="text-[9pt]">KRA PIN: {kraPin}</p>}{cafe && <p className="mt-1 text-[9pt]">Store #{sale.id.slice(0, 5).toUpperCase()} - {formatDateTime(sale.createdAt)}</p>}</div>
      <div className="my-[3mm] border-y border-dashed border-black py-[2mm] text-center text-[9pt]"><p className="text-[12pt] font-bold tracking-wide">{isProvisional ? 'PROVISIONAL RECEIPT' : 'SALES RECEIPT'}</p>{sale.cafeOrder && <p className="text-[11pt] font-bold">ORDER #{sale.cafeOrder.orderNumber}</p>}<p>{formatDateTime(sale.createdAt)}</p><p>Receipt: {sale.receiptNo}</p>{sale.cafeOrder?.orderType && <p>Order type: {sale.cafeOrder.orderType.replace('_', '-')}</p>}{sale.cafeOrder?.tableName && <p>Table: {sale.cafeOrder.tableName}</p>}{showCashier && <p>Cashier: {cashierName}</p>}{showCustomer && <p>Customer: {customerName}</p>}{isProvisional && <><p className="mt-2 border-y border-black py-1 font-bold">OFFLINE - SYNC PENDING</p><p className="mt-1 text-[9pt] font-bold leading-3">NOT AN OFFICIAL OR FISCAL RECEIPT</p></>}</div>
      <div>{sale.items.map((item) => <div key={item.id} className="break-inside-avoid border-b border-dotted border-zinc-400 py-1.5 text-[10pt]"><p className="break-words font-medium leading-tight">{item.productName}</p>{item.modifierNames?.map((name) => <p key={name} className="pl-2 text-[9pt]">+ {name}</p>)}{item.lineNotes && <p className="pl-2 text-[9pt]">Note: {item.lineNotes}</p>}{showItemSku && <p className="text-[9pt]">SKU: {item.productId.slice(0, 8).toUpperCase()}</p>}<div className="mt-0.5 flex items-baseline justify-between gap-2 text-[9pt] tabular-nums"><span className="whitespace-nowrap">{item.quantity} x {formatCurrency(parseFloat(item.unitPrice.toString()))}</span><span className="whitespace-nowrap text-right font-medium">{formatCurrency(parseFloat(item.totalPrice.toString()))}</span></div></div>)}</div>
      ...
      {showPayment && <div className="mt-[3mm] border-y border-dashed border-black py-[2mm] text-center"><p>Paid by: <span className="font-bold capitalize">{paymentLabel}</span></p><p className="font-bold">Amount paid: {formatCurrency(total)}</p>{sale.mpesaRef && <p>Reference: {sale.mpesaRef}</p>}{sale.mpesaDetails?.merchant && sale.mpesaDetails.mode !== 'stk' && <p>{sale.mpesaDetails.mode === 'till' ? 'Till' : 'PayBill'}: {sale.mpesaDetails.merchant}</p>}{sale.mpesaDetails?.accountReference && <p>Account: {sale.mpesaDetails.accountReference}</p>}</div>}
      ...
      {showQrCode && !isProvisional && <div className="mt-5 flex flex-col items-center justify-center"><ReceiptQrCode saleId={sale.id} receiptNo={sale.receiptNo} total={formatCurrency(total)} paymentMethod={sale.paymentMethod} createdAt={sale.createdAt} size={108} /><p className="mt-2 text-[9pt] leading-3">Scan for receipt details</p></div>}
