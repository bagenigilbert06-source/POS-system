...
function TrendLine({ trend }: { trend: MetricTrend }) {
  ...
  return (
    <p className={cn(
      ...
      <span>{trend.text ?? `${trend.value?.toFixed(1)}% ${trend.label ?? ''}`}</span>
    </p>
  )
}

export function MetricCard({ title, value, icon: Icon, description, href, trend, primaryMeta, status, warning, healthy, loading, linkLabel = 'View details' }: MetricCardProps) {
  ...
  const card = (
    <Card className={cn(
      ...
      <CardHeader className="relative z-[1] flex-row items-start justify-between space-y-0 px-4 pb-0 pt-4">
        ...
        <span className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border transition-colors', iconTone)}>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="mt-auto pt-3">
              ...
              ) : trend ? <TrendLine trend={trend} /> : primaryMeta ? <p className="min-h-5 text-xs font-medium text-[var(--dashboard-muted)]">{primaryMeta}</p> : null}
            </div>
            <p className="mt-1.5 line-clamp-2 min-h-4 text-[0.7rem] leading-4 text-[var(--dashboard-muted)]">{description}</p>
          </>
        )}
          ...
          <TooltipProvider delayDuration={300}>
            <Tooltip>
              <TooltipTrigger asChild>
                <span className="absolute bottom-3.5 right-3.5 flex items-center gap-1 rounded-md px-1.5 py-1 text-[0.64rem] font-semibold text-[var(--dashboard-muted)] opacity-70 transition-colors group-hover:text-[var(--dashboard-accent)] group-hover:opacity-100 group-focus-within:text-[var(--dashboard-accent)] group-focus-within:opacity-100" aria-hidden="true">
                  <span>{linkLabel}</span><ArrowUpRight className="h-3.5 w-3.5" />
                </span>
              </TooltipTrigger>
              <TooltipContent className="border border-[var(--dashboard-border)] bg-[var(--dashboard-chart-tooltip)] text-[var(--dashboard-text)] shadow-lg">View details</TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
      </CardContent>
  ...
  return href ? (
    <Link href={href} aria-label={`View ${title} details`} className="block h-full rounded-2xl outline-none focus-visible:ring-2 focus-visible:ring-[var(--dashboard-accent)] focus-visible:ring-offset-2">
      {card}
    </Link>
  ) : card
}
...
export function MetricCardSkeleton() {
  return <MetricCard title="Loading metric" value="" icon={Minus} description="" loading />
}
