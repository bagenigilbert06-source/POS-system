...
export function PosPinLoginForm() {
  ...
  const submit=(value=pin)=>{if(value.length!==6||pending)return;start(async()=>{setError('');const result=await unlockPosByPin(value);if('error' in result){setPin('');setError(result.error.includes('currently has an open shift') ? result.error : 'PIN couldn\'t unlock this POS. Check the PIN or ask a manager.');requestAnimationFrame(()=>inputRef.current?.focus());return;}router.replace('/dashboard/pos');router.refresh()})};
