...
async function paymentAuthorization() {
  ...
  const [activeBranch] = branchId
    ...
    : await db
        ...
        .limit(1);
  if (!activeBranch)
    throw new Error('No authorized branch is available for this POS');
  return {
    ...authorization,
...
async function manualAccountsForBranch(
  ...
  const configured = await db
    ...
    .where(
      and(
        eq(mpesaBusinessAccount.organizationId, organizationId),
        eq(mpesaBusinessAccount.branchId, branchId),
        eq(mpesaBusinessAccount.active, true),
        inArray(mpesaBusinessAccount.accountType, ['till', 'paybill'])
      )
    );
...
export async function setManualMpesaPayerPhone(
  requestId: string,
  value?: string | null
) {
  const authorization = await paymentAuthorization();
  ...
  await db
    ...
    .where(
      and(
        ...
        inArray(mpesaPaymentRequest.paymentMode, ['till', 'paybill']),
...
async function validatePharmacyPayment(
  authorization: Awaited<ReturnType<typeof paymentAuthorization>>,
  productIds: string[],
  workflow: z.input<typeof pharmacyWorkflowSchema>
) {
  ...
  const [items, policies] = await Promise.all([
    ...
    db
      ...
      .where(
        eq(pharmacyConfiguration.organizationId, authorization.organizationId)
      )
      .limit(1),
  ]);
  ...
  if (
    ...
    !authorization.permissions.includes(PermissionEnum.PRESCRIPTION_DISPENSE)
  )
    throw new Error(
      'This staff role is not allowed to dispense prescription medicines'
    );
  if (
    (policy?.prescriptionWorkflowEnabled ?? true) &&
    items.some((item) => item.prescriptionRequired) &&
    !workflow?.prescriptionReference
  )
    throw new Error(
      'Enter the prescription reference before requesting payment'
    );
  if (
    (policy?.restrictedItemWorkflowEnabled ?? true) &&
    items.some((item) => item.restrictedItem) &&
  ...
  if (items.some((item) => item.restrictedItem) && !workflow?.notes?.trim())
...
export async function initiateMpesaPayment(
  ...
  if (
    ...
    !authorization.permissions.includes(PermissionEnum.REWARDS_REDEEM)
  )
  ...
  if (!methods.includes('mpesa'))
    throw new Error('M-Pesa is not enabled for this workspace');

  const productIds = Array.from(
    new Set(data.items.map((item) => item.productId))
  );
  if (productIds.length !== data.items.length)
    throw new Error('Duplicate basket items are not allowed');
  ...
  for (const line of data.items) {
    ...
    if (
      ...
      throw new Error('A basket package is unavailable');
    const baseQuantity =
      line.quantity * (selectedPackage?.baseUnitQuantity ?? 1);
    if (!item?.active || item.stock < baseQuantity)
      throw new Error('A basket item is unavailable or has insufficient stock');
    subtotal +=
  ...
  const unroundedTotal = Number(
    (gross + data.shippingAmount - data.discountAmount).toFixed(2)
  );
  if (unroundedTotal < 1 || unroundedTotal > 150000)
    throw new Error('M-Pesa amount must be between KES 1 and KES 150,000');
  let exactTotal = data.roundoffEnabled
  ...
  const [existing] = await db
    ...
    .where(
      and(
        ...
        eq(mpesaPaymentRequest.idempotencyKey, data.idempotencyKey)
      )
    )
    .limit(1);
  if (existing)
    return {
      ...
      message: existing.resultDescription,
  ...
  await db.transaction(async (tx) => {
    const [lockedShift] = await tx
      ...
      .where(
        and(
          ...
          eq(posSession.orgId, orgId),
          eq(posSession.branchId, branchId),
          eq(posSession.status, 'open')
        )
      )
      .limit(1)
    ...
    const reservation = data.customerId
      ? await reserveRewardsForPayment(tx, {
      ...
      : { externalAmountReduction: 0 };
    exactTotal = data.roundoffEnabled
      ? calculateMpesaAmount(
          unroundedTotal - reservation.externalAmountReduction
        ).amount
      : Number(
          (unroundedTotal - reservation.externalAmountReduction).toFixed(2)
        );
    if (exactTotal < 1)
      throw new Error('M-Pesa amount after rewards must be at least KES 1');
  ...
  try {
    ...
    await db
      ...
      .set({
        ...
        resultDescription: response.CustomerMessage,
        updatedAt: new Date(),
      })
...
export async function initiateMpesaPaybillPayment(
  ...
  if (!selectedAccount)
    throw new Error(
      'No active Till or PayBill account is configured for this branch'
    );
  ...
  if (existing)
    return {
      ...
      shortcode: selectedAccount.shortcode,
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.insert(mpesaPaymentRequest).values({
      ...
      resultDescription: 'Awaiting payment confirmation',
      expiresAt,
    });
  });
  return {
...
export async function getMpesaPaymentStatus(requestId: string) {
  ...
  const [request] = await db
    .select({
      ...
      amount: mpesaPaymentRequest.amount,
      receiptNumber: mpesaPaymentRequest.receiptNumber,
      message: mpesaPaymentRequest.resultDescription,
  ...
  if (
    ['SENDING_STK', 'AWAITING_CUSTOMER', 'AWAITING_CONFIRMATION'].includes(
      request.status
    ) &&
    ...
    await db
      ...
      .set({
        status: 'EXPIRED',
        resultDescription: 'No M-Pesa confirmation was received in time',
        updatedAt: new Date(),
      })
      .where(
        and(
          eq(mpesaPaymentRequest.id, request.id),
          eq(mpesaPaymentRequest.status, request.status)
        )
      );
    return {
      ...request,
      status: 'EXPIRED',
      message: 'No M-Pesa confirmation was received in time',
    };
  }
  return { ...request, amount: Number(request.amount) };
}

/** Cancels an unconfirmed M-Pesa intent before the cashier changes payment method. */
export async function cancelMpesaPayment(requestId: string) {
  const authorization = await paymentAuthorization();
  const [cancelled] = await db
    .update(mpesaPaymentRequest)
    .set({
      status: 'CANCELLED',
      resultDescription: 'Cancelled by cashier before confirmation',
      updatedAt: new Date(),
    })
    .where(
      and(
  ...
  const [current] = await db
    .select({ status: mpesaPaymentRequest.status })
    .from(mpesaPaymentRequest)
    .where(
      and(
        eq(mpesaPaymentRequest.id, requestId),
  ...
  if (['CANCELLED', 'FAILED', 'EXPIRED'].includes(current.status))
    return { cancelled: true as const };
  throw new Error(
    'This M-Pesa payment has already been confirmed and cannot be switched.'
  );
}
...
export async function findManualMpesaPayment(requestId: string) {
  ...
  const [account] = await db
    ...
    .where(
      and(
        eq(mpesaBusinessAccount.organizationId, intent.organizationId),
        eq(mpesaBusinessAccount.branchId, intent.branchId!),
        eq(mpesaBusinessAccount.accountType, intent.paymentMode),
        eq(mpesaBusinessAccount.active, true)
      )
    )
    .limit(1);
  if (!account)
    throw new Error('The branch M-Pesa account is no longer active');
  const candidates = await db
    .select()
    .from(mpesaIncomingPayment)
    .where(
      and(
        eq(mpesaIncomingPayment.organizationId, intent.organizationId),
        eq(mpesaIncomingPayment.branchId, intent.branchId!),
        eq(mpesaIncomingPayment.shortcode, account.shortcode),
        intent.phone ? eq(mpesaIncomingPayment.phone, intent.phone) : undefined,
        intent.paymentMode === 'paybill'
          ? eq(mpesaIncomingPayment.accountReference, intent.accountReference!)
          : undefined,
        gte(mpesaIncomingPayment.createdAt, intent.createdAt),
        isNull(mpesaIncomingPayment.matchedRequestId)
      )
    )
    .orderBy(desc(mpesaIncomingPayment.createdAt))
    .limit(5);
  const exact = candidates.filter(
    (candidate) => Number(candidate.amount) === Number(intent.amount)
  );
  if (exact.length > 1)
    return { status: 'ambiguous' as const, count: exact.length };
  if (!exact.length) {
    ...
    return nearest
      ? {
          ...
          received: Number(nearest.amount),
          receiptNumber: nearest.transactionId,
        }
  ...
  await db.transaction(async (tx) => {
    const [claimedReceipt] = await tx
      .update(mpesaIncomingPayment)
      .set({
        matchedRequestId: intent.id,
        matchedAt: new Date(),
        matchedBy: authorization.userId,
        status: 'MATCHED_PENDING_FINALIZATION',
      })
      .where(
        and(
          eq(mpesaIncomingPayment.id, payment.id),
          isNull(mpesaIncomingPayment.matchedRequestId)
        )
      )
      .returning({ id: mpesaIncomingPayment.id });
    if (!claimedReceipt)
      throw new Error('This M-Pesa payment has already been used.');
    const [claimedIntent] = await tx
      .update(mpesaPaymentRequest)
      .set({
        receiptNumber: payment.transactionId,
        resultCode: '0',
        resultDescription: 'Payment found and verified',
        status: 'CONFIRMED',
        callbackPayload: payment.payload,
        completedAt: new Date(),
        updatedAt: new Date(),
      })
      .where(
        and(
          eq(mpesaPaymentRequest.id, intent.id),
          eq(mpesaPaymentRequest.status, 'AWAITING_CONFIRMATION')
        )
      )
      .returning({ id: mpesaPaymentRequest.id });
...
export async function getFinalizedMpesaSale(requestId: string) {
  ...
  const [completedSale] = await db
    ...
    .where(
      and(
        eq(sale.id, request.saleId),
        eq(sale.orgId, authorization.organizationId)
      )
    )
    .limit(1);
  if (!completedSale || completedSale.paymentMethod !== 'mpesa')
    throw new Error('Finalized M-Pesa sale was not found');
  const items = await db
    .select({ saleItemId: saleItem.id, productId: saleItem.productId })
    .from(saleItem)
    .where(
      and(
        eq(saleItem.saleId, completedSale.id),
  ...
  return {
    ...
    etims: {
      ...
      message: 'Sale completed. eTIMS submission is processed independently.',
    },
  };
}
