...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await OnboardingService.saveStep(session.user.id, body.data.stepId, body.data.data)
    ...
    return NextResponse.json({ success: true, currentStep: result.state.currentStep, lastSavedAt: result.state.lastSavedAt })
  } catch (error) {
