  ...
  return (
    <div className="min-w-[190px] rounded-xl border border-[var(--dashboard-border)] bg-[var(--dashboard-chart-tooltip)] p-3 text-xs shadow-xl">
      ...
      <div className="mt-2.5 space-y-2">
        <div className="flex items-center justify-between gap-6"><span className="text-[var(--dashboard-muted)]">Sales</span><span className="font-semibold tabular-nums text-[var(--dashboard-text)]">{formatCurrency(point.revenue, currency)}</span></div>
        ...
        <div className="border-t border-[var(--dashboard-border)] pt-2 flex items-center justify-between gap-6"><span className="text-[var(--dashboard-muted)]">After expenses</span><span className={`font-bold tabular-nums ${point.net >= 0 ? 'text-[var(--dashboard-success)]' : 'text-[var(--dashboard-danger)]'}`}>{formatCurrency(point.net, currency)}</span></div>
      </div>
    </div>
  )
}
    ...
    <Card className="flex min-h-[430px] flex-col overflow-hidden rounded-2xl border border-[var(--dashboard-border)] bg-[var(--dashboard-surface)] text-[var(--dashboard-text)] shadow-[0_1px_2px_rgba(16,24,40,.04)]">
      <CardHeader className="flex-row items-start justify-between gap-4 space-y-0 border-b border-[var(--dashboard-border)] px-5 py-4">
        <div className="min-w-0">
          <h2 className="text-[0.95rem] font-bold tracking-tight">Sales by day</h2>
          <p className="mt-0.5 text-xs text-[var(--dashboard-muted)]">Each bar shows how much you sold that day.</p>
        </div>
      ...
      <CardContent className="flex flex-1 flex-col px-5 pb-4 pt-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="grid flex-1 grid-cols-3 divide-x divide-[var(--dashboard-border)]">
            <div className="pr-3">
              <p className="text-[0.62rem] font-semibold uppercase tracking-[0.11em] text-[var(--dashboard-muted)]">Sales</p>
            ...
            <div className="pl-3">
              <p className="text-[0.62rem] font-semibold uppercase tracking-[0.11em] text-[var(--dashboard-muted)]">After expenses</p>
              ...
              <p className={`mt-0.5 text-[0.62rem] font-medium ${net >= 0 ? 'text-[var(--dashboard-success)]' : 'text-[var(--dashboard-danger)]'}`}>{net >= 0 ? 'Sales minus expenses' : 'Expenses exceed sales'}</p>
            </div>
          </div>
        ...
        {hasData ? (
          <>
            <div className="mt-5 h-[235px] min-h-[235px] w-full" role="img" aria-label={`Daily sales for ${periodLabel}`}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }} barCategoryGap="28%">
                  <CartesianGrid vertical={false} stroke="var(--dashboard-border)" strokeOpacity={0.5} />
                  <XAxis dataKey="label" axisLine={false} tickLine={false} minTickGap={40} tick={{ fill: 'var(--dashboard-muted)', fontSize: 10 }} dy={9} />
                  ...
                  <Tooltip cursor={{ fill: 'var(--dashboard-accent-soft)' }} content={<PerformanceTooltip currency={currency} />} isAnimationActive={false} />
                  <Bar dataKey="revenue" name="Sales" fill="var(--dashboard-accent-cta)" radius={[4, 4, 0, 0]} maxBarSize={30} isAnimationActive={false} />
                </BarChart>
              </ResponsiveContainer>
            </div>
            <p className="mt-2 border-t border-[var(--dashboard-border)] pt-3 text-[0.68rem] text-[var(--dashboard-muted)]"><strong className="text-[var(--dashboard-text)]">{daysWithSales}</strong> {daysWithSales === 1 ? 'day' : 'days'} with sales in this period. Hover a bar to see the details.</p>
          </>
        ) : (
          <div className="flex min-h-[250px] flex-1 flex-col items-center justify-center text-center">
            ...
            <p className="mt-1 max-w-sm text-xs leading-5 text-[var(--dashboard-muted)]">Your daily sales will appear here after a completed transaction.</p>
          </div>
        )}

        <div className="sr-only" aria-live="polite">For {periodLabel}: sales {formatCurrency(revenue, currency)}, expenses {formatCurrency(expenses, currency)}, amount after expenses {formatCurrency(net, currency)}.</div>
      </CardContent>
    </Card>
  )
}
