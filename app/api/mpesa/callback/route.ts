...
export async function POST(request: NextRequest) {
  ...
  const rawPaidPhone = String(metadata.get('PhoneNumber') || '')
  let paidPhone = rawPaidPhone
  try { paidPhone = normalizeKenyanPhone(rawPaidPhone) } catch { /* invalid provider data cannot verify */ }
  const successful = resultCode === 0 && receiptNumber.length > 0 && Number(payment.amount) === paidAmount && paidPhone === payment.phone
  ...
  const failureStatus = resultCode === 1032 ? 'CANCELLED' : resultCode === 1037 ? 'EXPIRED' : 'FAILED'
