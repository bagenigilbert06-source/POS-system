  ...
  const metrics = [
    {
      ...
      detail: openCount
        ? `${openCount} shift${openCount === 1 ? '' : 's'} awaiting reconciliation`
        : 'No register is currently open',
      icon: Banknote,
      tone: openCount ? 'positive' : 'warning',
      href: '#supervisor-actions',
    },
    {
      label: "Today's sales",
      value: formatCurrency(salesTotal, organization.currency),
      detail: `${formatNumber(Number(data.summary.salesToday.count))} completed receipts`,
      icon: ReceiptText,
      tone: 'positive',
    ...
    {
      label: 'Refunds today',
      value: formatCurrency(refundTotal, organization.currency),
      detail: `${formatNumber(Number(data.summary.refundsToday.count))} credit notes issued`,
      icon: AlertTriangle,
      tone: refundTotal ? 'warning' : 'positive',
      href: '#recent-credit-notes',
    },
    {
      label: 'Stock lost today',
      value: `${formatNumber(lossQuantity)} units`,
      detail: `${formatNumber(Number(data.summary.lossesToday.count))} loss records · ${formatCurrency(Number(data.summary.lossesToday.totalCost), organization.currency)}`,
      icon: Boxes,
      tone: lossQuantity ? 'warning' : 'positive',
      href: '#recent-stock-losses',
    },
  ] as const;
    ...
    <div className="dashboard-overview mx-auto w-full max-w-[1440px] space-y-5 pb-8">
      <header className="dashboard-welcome flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Link
            href="/dashboard/pos"
            className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-[#ffd60a] px-5 text-sm font-semibold text-[#0b0b0d] shadow-sm transition-colors hover:bg-[#ffdf3a]"
          >
          ...
          <Link
            href="/dashboard/sales"
            className="inline-flex min-h-10 items-center gap-2 rounded-lg border px-5 text-sm font-semibold transition-colors"
          >
      ...
      {!openCount && (
        <section className="flex flex-col gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-amber-950 shadow-sm dark:border-amber-900/50 dark:bg-amber-950/20 dark:text-amber-100 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-300">
              <AlertTriangle className="h-4 w-4" />
            </span>
            <div>
              <h2 className="text-sm font-bold">No register is open</h2>
              <p className="mt-1 text-xs leading-5 opacity-80">
                Open a register before counter sales begin so cash activity can
...
function MetricCard({
  ...
  return (
    <Link
      href={href}
      className={`dashboard-metric-card ${tone === 'warning' ? 'metric-alert' : 'metric-positive'} group rounded-xl border px-4 py-3.5 outline-none focus-visible:ring-2 focus-visible:ring-[#d6a800]`}
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className={
            tone === 'warning'
              ? 'flex h-9 w-9 items-center justify-center rounded-lg bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300'
              : 'flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300'
          }
        >
          <Icon className="h-4 w-4" />
        </span>
      </div>
      <p className="mt-3 truncate text-2xl font-bold tabular-nums tracking-tight">
        {value}
      </p>
...
function History({
  ...
  title: string;
  empty: string;
  rows: { title: string; detail: string; value: string; status: string }[];
}) {
  return (
    <article
      id={id}
      className="scroll-mt-24 overflow-hidden rounded-2xl border bg-white shadow-[0_4px_14px_rgba(16,24,40,.05)] dark:bg-card"
    >
      <div className="flex items-center gap-3 border-b px-5 py-4">
        <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#fff7d6] text-[#9a6700] dark:bg-[rgba(255,214,10,.08)] dark:text-[#ffd60a]">
          <Icon className="h-4 w-4" />
        </span>
        <div>
