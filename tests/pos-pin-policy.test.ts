...
import { findPosPinOwners, validatePosPin, POS_PIN_LENGTH, POS_PIN_LOCK_SECONDS, POS_PIN_MAX_ATTEMPTS } from '../lib/pos/pin-policy'
assert.equal(POS_PIN_LENGTH, 6)
assert.equal(POS_PIN_MAX_ATTEMPTS, 5)
assert.equal(POS_PIN_LOCK_SECONDS, 30)
...
assert.equal(validatePosPin('012345'), null, 'leading zeroes must remain valid string PIN digits')
