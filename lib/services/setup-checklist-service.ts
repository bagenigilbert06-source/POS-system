...
export async function getSetupChecklist(organizationId: string, enabledModules: string[]) {
  ...
  const items: SetupChecklistItem[] = [
    ...
    ...(usesSuppliers ? [{ id: 'suppliers', title: 'Add a supplier', description: 'Create the first supplier record used on Stock Intake.', href: '/dashboard/settings', completed: suppliers.value > 0 }] : []),
