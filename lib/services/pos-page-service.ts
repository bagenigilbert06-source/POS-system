...
import { branch, businessSettings, cashMovement, category, customer, inventoryBalance, posPinCredential, posSession, posTerminal, product, sale, salesReturn } from '@/lib/db/schema'
...
export async function getPosPageData(authorization: AuthorizationContext, includeCustomers: boolean) {
  ...
  const [products, categories, customers, settingsRows, sessionRows, branchRows, pinRows] = await Promise.all([
