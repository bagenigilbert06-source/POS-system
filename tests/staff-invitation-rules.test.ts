...
assert.match(service, /STAFF_INVITATION_EXPIRY_MINUTES = 10/)
assert.match(service, /STAFF_INVITATION_EXPIRY_MINUTES \* 60_000/)
...
assert.doesNotMatch(authConfig, /backgroundTasks/)
...
assert.match(creation, /await invitationRedirectUrl\(\)/)
assert.match(creation, /x-forwarded-host/)
assert.doesNotMatch(creation, /https:\/\/pesaby\.vercel\.app/)
