    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[94vh] overflow-y-auto border-[var(--dashboard-border)] bg-[var(--dashboard-background)] p-0 sm:max-w-[1200px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 p-6">
          ...
          <div className="flex justify-end gap-3 border-t border-[var(--dashboard-border)] pt-5"><button type="button" onClick={() => onOpenChange(false)} className="inline-flex h-10 items-center rounded-lg border border-[var(--dashboard-border)] bg-[var(--dashboard-surface)] px-4 text-sm font-semibold text-[var(--dashboard-text)]">Cancel</button><button type="submit" disabled={isLoading} className="inline-flex h-10 items-center gap-2 rounded-lg bg-[var(--dashboard-accent-cta)] px-5 text-sm font-semibold text-[var(--dashboard-accent-cta-ink)] hover:bg-[var(--dashboard-accent-cta-hover)] disabled:opacity-60">{isLoading && <Loader2 className="h-4 w-4 animate-spin" />}Update Employee</button></div>
        </form>
      </DialogContent>
    </Dialog>
  )
