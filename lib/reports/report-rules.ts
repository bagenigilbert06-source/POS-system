export type ReportPeriodPreset = 'today' | 'yesterday' | '7d' | '30d' | 'this_month' | 'last_month' | 'custom'
...
export function inclusivePeriodDays(from: string, to: string) {
  if (!isDateKey(from) || !isDateKey(to) || from > to) throw new Error('Invalid report period')
...
export function resolveReportPeriod(preset: string, customFrom: string | undefined, customTo: string | undefined, today: string) {
  ...
  if (preset === 'yesterday') { const day = moveDateKey(today, -1); return { from: day, to: day } }
  ...
  if (preset === 'custom' && isDateKey(customFrom) && isDateKey(customTo) && customFrom <= customTo) return { from: customFrom, to: customTo }
  return { from: monthStart, to: today }
}

/** Sales totals are stored after discounts; completed refunds are deducted once. */
export function calculateNetSales(collectedAfterDiscounts: number, completedRefunds: number) {
...
export function selectReportExportRows(section: string, rows: { summary: ReportCsvRow[]; monthly: ReportCsvRow[]; payments: ReportCsvRow[]; products: ReportCsvRow[]; inventory: ReportCsvRow[]; shifts: ReportCsvRow[] }) {
  ...
  return [...rows.summary, [], ...rows.monthly, [], ...rows.payments, [], ...rows.products]
}
