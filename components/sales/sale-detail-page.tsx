...
export function SaleDetailPage({ detail, settings }: { detail: Detail; settings: Settings }) {
  ...
  return (
    <div className="sales-workspace mx-auto w-full max-w-[1180px] space-y-5 pb-10">
      ...
      {detail.etims && <section className="rounded-xl border bg-card p-4 shadow-sm dark:border-white/10"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[.12em] text-muted-foreground">eTIMS fiscal status</p><p className="mt-1 text-sm font-semibold">{detail.etims.status === 'ACCEPTED' || detail.etims.status === 'CREDITED' ? 'Accepted' : detail.etims.status === 'FAILED' ? 'Action required' : 'Pending submission'}</p>{detail.etims.errorMessage && <p className="mt-1 text-xs text-destructive">{detail.etims.errorMessage}</p>}</div>{detail.etims.invoiceNumber && <div className="text-right"><p className="text-xs text-muted-foreground">Invoice reference</p><p className="mt-1 font-mono text-xs font-semibold">{detail.etims.invoiceNumber}</p></div>}</div></section>}
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1.55fr)_minmax(320px,.75fr)]">
        <div className="space-y-5">
          ...
          {detail.pharmacyRecord && <section className="rounded-xl border bg-card p-5 shadow-sm dark:border-white/10"><h2 className="text-base font-semibold">Dispensing record</h2><div className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><div><p className="text-xs text-muted-foreground">Prescription reference</p><p className="mt-1 font-medium">{detail.pharmacyRecord.prescriptionReference || 'Restricted-item record'}</p></div><div><p className="text-xs text-muted-foreground">Prescriber reference</p><p className="mt-1 font-medium">{detail.pharmacyRecord.prescriberReference || 'Not recorded'}</p></div></div></section>}
...
function LineItem({ item, canViewCost }: { item: Detail['items'][number]; canViewCost: boolean }) {
  ...
  const medicineDescription = [item.genericName, item.strength, item.dosageForm].filter(Boolean).join(' · ')
  const hasDetails = Boolean(item.sku || medicineDescription || item.lotAllocations.length || (canViewCost && unitCost !== undefined))
  return <div className="px-5 py-4"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="font-medium">{titleCase(item.productName)}</p>{medicineDescription && <p className="mt-1 text-xs font-medium text-foreground">{medicineDescription}</p>}<p className="mt-1 text-xs text-muted-foreground">{item.quantity} × {formatCurrency(item.unitPrice)} · {titleCase(item.categoryName ?? 'Uncategorized')}</p>{item.lotAllocations.length > 0 && <p className="mt-1 text-[11px] text-muted-foreground">Batch {item.lotAllocations.map((allocation) => allocation.lotNumber).join(', ')}</p>}</div><p className="shrink-0 font-semibold tabular-nums">{formatCurrency(item.totalPrice)}</p></div>{hasDetails && <div className="mt-2"><button type="button" onClick={() => setExpanded((value) => !value)} aria-expanded={expanded} className="inline-flex items-center gap-1 text-xs font-medium text-muted-foreground transition-colors hover:text-foreground focus-visible:rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">Details <ChevronDown className={`h-3 w-3 transition-transform ${expanded ? 'rotate-180' : ''}`} /></button>{expanded && <div className="mt-2 space-y-1 rounded-lg bg-secondary/40 px-3 py-2 text-[11px] text-muted-foreground">{item.sku && <p className="break-all"><span className="font-medium">SKU:</span> {item.sku}</p>}{item.manufacturer && <p><span className="font-medium">Manufacturer:</span> {item.manufacturer}</p>}{item.prescriptionRequired && <p className="font-medium">Prescription required</p>}{item.restrictedItem && <p className="font-medium text-amber-700">Restricted medicine approval recorded</p>}{item.lotAllocations.map((allocation) => <p key={allocation.lotNumber}><span className="font-medium">Batch {allocation.lotNumber}:</span> {Number(allocation.quantity)} units{allocation.expiresAt ? ` · expires ${new Date(allocation.expiresAt).toLocaleDateString()}` : ''}</p>)}{canViewCost && unitCost !== undefined && <p><span className="font-medium">Unit cost:</span> {formatCurrency(unitCost)}</p>}</div>}</div>}</div>
}
