...
function HourTooltip({
  ...
  active?: boolean;
  payload?: ReadonlyArray<{ payload?: HourPoint }>;
  currency: string;
}) {
...
export function BusiestHoursCard({
  currency,
  reportDate,
  sales,
}: BusiestHoursCardProps) {
  ...
  const view = useMemo(() => {
    ...
    const periodSales = sales.filter(
      (sale) => sale.date >= firstDate && sale.date <= reportDate
    );
    const points = Array.from({ length: 24 }, (_, hour): HourPoint => {
      ...
      const transactions = matching.reduce(
        (sum, sale) => sum + sale.transactions,
        0
      );
      return {
        hour,
        revenue,
        transactions,
        activity: transactions / period,
        label: hourRange(hour),
      };
    });
    ...
    const peak = active.reduce<HourPoint | null>((best, point) => {
      if (
        !best ||
        point.activity > best.activity ||
        (point.activity === best.activity && point.revenue > best.revenue)
      )
        return point;
      return best;
    }, null);
    ...
    return {
      ...
      peak,
      quietest,
      transactions: periodSales.reduce(
        (sum, sale) => sum + sale.transactions,
        0
      ),
    };
  ...
  return (
    <article className="flex h-full min-h-[430px] flex-col overflow-hidden rounded-xl border border-[var(--dashboard-border)] bg-[var(--dashboard-surface)] text-[var(--dashboard-text)] shadow-dark-sm">
      <div className="flex h-16 items-center justify-between gap-3 border-b border-[var(--dashboard-border)] px-5">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-[var(--dashboard-muted)]">
              When customers are buying most
            </p>
          </div>
        </div>
        <div className="relative shrink-0">
          <button
            ...
            aria-label="Busiest hours period"
            onClick={() => setPeriodOpen((open) => !open)}
            className="flex h-8 min-w-[88px] items-center justify-between gap-2 rounded-md border border-[var(--dashboard-border)] bg-[var(--dashboard-surface)] px-2.5 text-[0.68rem] font-semibold text-[var(--dashboard-text)] outline-none transition-colors hover:border-[var(--dashboard-accent-soft-border)] focus:ring-1 focus:ring-[var(--dashboard-accent-soft-border)]"
          >
            ...
            >
              {PERIOD_OPTIONS.map((option) => (
                <button
                  ...
                  className="block w-full rounded-md px-2 py-1.5 text-left text-xs font-medium text-[var(--dashboard-text)] outline-none hover:bg-[var(--dashboard-accent-soft)] hover:text-[var(--dashboard-accent)] focus:bg-[var(--dashboard-accent-soft)] focus:text-[var(--dashboard-accent)]"
                >
                  {option.label}
                </button>
              ))}
      ...
      {view.peak ? (
        <div className="flex flex-1 flex-col px-5 pb-4 pt-4">
          <div className="flex items-end justify-between gap-4">
            <div>
              <p className="text-[0.62rem] font-bold uppercase tracking-[0.12em] text-[var(--dashboard-muted)]">
                Peak time
              </p>
              <p className="mt-1 text-[1.2rem] font-bold leading-none tracking-tight">
                {view.peak.label}
              </p>
            </div>
            <div className="text-right">
              <p className="text-[0.82rem] font-bold tabular-nums">
                {formatCurrency(view.peak.revenue, currency)}
              </p>
              <p className="mt-1 text-xs text-[var(--dashboard-muted)]">
                {formatNumber(view.peak.transactions)}{' '}
                {view.peak.transactions === 1 ? 'transaction' : 'transactions'}
              </p>
            </div>
          </div>
          ...
          <div className="mt-5 flex items-center justify-between gap-3">
            ...
            <p className="text-[0.65rem] text-[var(--dashboard-muted)]">
              {period === 1
          ...
          <div
            ...
            role="img"
            aria-label={`Hourly sales activity. Peak time is ${view.peak.label}.`}
          >
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Tooltip
                  ...
                  content={<HourTooltip currency={currency} />}
                />
                <Bar
                  dataKey="activity"
                  radius={[4, 4, 2, 2]}
                  minPointSize={view.transactions ? 0 : undefined}
                ...
                >
                  {view.points.map((point) => (
                    <Cell
                      key={point.hour}
                      fill={
                        point.hour === view.peak?.hour
                          ? 'var(--dashboard-accent)'
                          : 'var(--dashboard-muted)'
                      }
                      fillOpacity={point.hour === view.peak?.hour ? 1 : 0.24}
                    />
                  ))}
                </Bar>
              </BarChart>
          ...
          <dl className="mt-auto grid grid-cols-2 divide-x divide-[var(--dashboard-border)] border-t border-[var(--dashboard-border)] pt-3.5">
            <div className="pr-4">
              <dt className="text-[0.62rem] font-semibold uppercase tracking-[0.08em] text-[var(--dashboard-muted)]">
                Busiest
              </dt>
              <dd className="mt-1 text-[0.82rem] font-bold">
                {view.peak.label}
              </dd>
            </div>
            <div className="pl-4">
        ...
        <div className="flex flex-1 flex-col items-center justify-center px-6 py-8 text-center">
          ...
          <p className="mt-1 max-w-[260px] text-xs leading-5 text-[var(--dashboard-muted)]">
            Busiest hours will appear as more sales are completed.
          </p>
        </div>
      )}
    </article>
