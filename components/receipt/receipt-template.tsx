...
export function ReceiptTemplate({
  ...
  if (layout === 'thermal') {
    ...
    return <div className="receipt-paper mx-auto w-[80mm] max-w-full bg-white px-[4mm] py-[6mm] font-mono text-[11px] leading-5 text-zinc-900 print:w-full print:max-w-none print:px-[3mm] print:py-[5mm]">
      ...
      {showPayment && <p className="mt-5 text-center">Payment: <span className="font-bold capitalize">{sale.paymentMethod}</span>{sale.mpesaRef ? ` · ${sale.mpesaRef}` : ''}</p>}
      {showQrCode && <div className="mt-5 flex flex-col items-center justify-center"><ReceiptQrCode saleId={sale.id} receiptNo={sale.receiptNo} total={formatCurrency(total)} paymentMethod={sale.paymentMethod} createdAt={sale.createdAt} /><p className="mt-2 text-[8px] leading-3 text-zinc-400">Scan for receipt details</p></div>}
      <div className="mt-5 border-t-2 border-dotted border-zinc-900 pt-4 text-center"><p className="font-bold">{receiptFooter}</p><p className="mt-1.5 text-[8px] leading-3 text-zinc-400">Transaction: {sale.id.slice(0, 8).toUpperCase()}</p></div>
    </div>
  }
