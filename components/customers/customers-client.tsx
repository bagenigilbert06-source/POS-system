...
import {
  useCustomersStore,
  OptimisticItem,
} from '@/lib/stores/customers-store';
...
import {
  ...
  GoogleMapsMark,
  PhoneMark,
} from '@/components/ui/contact-marks';
...
export function CustomersClient({
  ...
  useEffect(
    () => setCanonical(initialCustomers),
    [initialCustomers, setCanonical]
  );
  ...
  useEffect(() => {
    ...
    const serverItems = [...canonical, ...initialCustomers].filter(
      (c, index, items) => items.findIndex((x) => x.id === c.id) === index
    );
    const serverFiltered = serverItems.filter(
      (c) => !mappedOptimistic.some((m) => m.id === c.id)
    );
  ...
  const initials = (value: string) =>
    value
      ...
      .join('')
      .toUpperCase() || '?';
  const person = cafeMode ? 'guest' : 'customer';
  const people = cafeMode ? 'guests' : 'customers';
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      useCustomersStore
        .getState()
        .setCanonical(canonical.filter((item) => item.id !== deleteTarget.id));
      notify.success(`${cafeMode ? 'Guest' : 'Customer'} deleted`);
    ...
    <TooltipProvider delayDuration={250}>
      <div className="overflow-hidden rounded-xl bg-white shadow-[0_8px_30px_rgba(15,23,42,0.06)] dark:bg-card/80 dark:shadow-sm">
        ...
        <div className="flex flex-col gap-3 bg-slate-50/60 p-3 dark:bg-white/[0.015] sm:flex-row sm:items-center">
          <div className="relative w-full sm:max-w-md">
            ...
            {search && (
              <button
                ...
                className="absolute right-2 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
              >
                <X className="h-4 w-4" />
              </button>
            )}
          ...
          <span
            className="text-xs text-muted-foreground sm:ml-1 sm:mr-auto"
            aria-live="polite"
          >
          ...
          >
            ...
            {cafeMode ? 'Add Guest' : 'Add Customer'}
          </Link>
        </div>
          ...
          <div className="flex min-h-72 flex-col items-center justify-center border-t border-dashed border-border bg-card/70 px-6 py-12 text-center">
            ...
            <p className="mt-1 max-w-sm text-sm text-muted-foreground">
              {search
                ...
                : `Add a ${person} once, then select them again during checkout for faster service.`}
            </p>
            {!search && (
              <Link
                href="/dashboard/customers/new"
            ...
            <div className="divide-y divide-border dark:divide-white/[0.07]">
              {filtered.map((c) => (
                ...
                >
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              ...
                              >
                                <LockKeyhole
                                  className="h-3 w-3"
                                  aria-label="Partially hidden name"
                                />
                              </span>
                            </TooltipTrigger>
                            <TooltipContent>
                              Name appears partially hidden for privacy
                            </TooltipContent>
                          </Tooltip>
                        )}
                      </p>
                  ...
                  <div className="hidden min-w-0 items-center gap-1.5 text-sm text-muted-foreground lg:flex">
                    {c.email ? (
                      <>
                        ...
                        <a
                          href={`mailto:${c.email}`}
                          className="truncate hover:text-foreground hover:underline focus-visible:rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
                        >
                          {c.email}
                        </a>
                      </>
                  ...
                  <span className="hidden text-right text-[11px] text-muted-foreground lg:block">
                  ...
                  <div className="flex items-center justify-end">
                    ...
                    <div className="flex items-center rounded-lg border border-border bg-background/70 p-0.5 shadow-sm dark:border-white/10 dark:bg-white/[0.025]">
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Link
                            href={`/dashboard/customers/${c.id}?mode=view`}
                            className="rounded-md p-1.5 text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                            aria-label={`View ${displayName(c.name)}`}
                          >
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Link
                            ...
                            className="rounded-md p-1.5 text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                            aria-label={`Edit ${displayName(c.name)}`}
                          >
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button
                            ...
                            className="rounded-md p-1.5 text-muted-foreground transition-colors hover:bg-red-50 hover:text-red-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 dark:hover:bg-red-400/10 dark:hover:text-red-300"
                            aria-label={`Delete ${displayName(c.name)}`}
                          >
                  ...
                  {(c as any).optimistic && (c as any).status === 'failed' && (
                    <div className="col-span-full flex items-center gap-2 pb-2 pl-[52px] md:pl-0">
                      <button
                        ...
                        onClick={async () => {
                          ...
                          const opt = optimistic.find(
                            (o) => o.tempId === c.id
                          ) as OptimisticItem | undefined;
                          if (!opt) return;
                          try {
                            useCustomersStore
                              .getState()
                              .addOptimistic({ ...opt, status: 'pending' });
                            ...
                            useCustomersStore
                              .getState()
                              .replaceOptimistic(opt.tempId, res);
                          } catch (err) {
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          useCustomersStore.getState().removeOptimistic(c.id)
                        }
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {deleteTarget ? displayName(deleteTarget.name) : `this ${person}`}{' '}
              from the {people} directory. Existing sales records are not
              changed.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
