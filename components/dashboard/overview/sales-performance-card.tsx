...
function PerformanceTooltip({
  ...
  const point = payload?.[0]?.payload as (SalesPoint & { fullDate: string }) | undefined
  if (!active || !point) return null

  return (
    <div className="min-w-44 rounded-xl border border-[var(--dashboard-border)] bg-[var(--dashboard-chart-tooltip)] p-3 shadow-xl">
      <p className="text-[0.68rem] font-semibold text-[var(--dashboard-muted)]">{point.fullDate}</p>
...
export function SalesPerformanceCard({ currency, data }: SalesPerformanceCardProps) {
  const [range, setRange] = useState<Range>('month')
  const [rangeOpen, setRangeOpen] = useState(false)

  const view = useMemo(() => {
    const latest = data.at(-1)
    const days = range === 'today' ? 1 : range === '7-days' ? 7 : range === '30-days' ? 30 : latest ? parseDate(latest.date).getDate() : 1
    ...
    const transactions = current.reduce((sum, point) => sum + point.transactions, 0)
    const previousRevenue = previous.reduce((sum, point) => sum + point.revenue, 0)
    const change = previousRevenue > 0 ? ((revenue - previousRevenue) / previousRevenue) * 100 : revenue > 0 ? null : 0
    ...
    return {
      ...
      points: current.map((point) => ({
        ...point,
        label: parseDate(point.date).toLocaleDateString('en-KE', days <= 7 ? { weekday: 'short' } : { day: 'numeric', month: 'short' }),
        fullDate: parseDate(point.date).toLocaleDateString('en-KE', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' }),
      })),
    }
  }, [data, range])

  const comparison = view.change === null
    ? { text: 'No previous-period comparison', tone: 'text-[var(--dashboard-muted)]' }
    : view.change > 0
      ? { text: `+${view.change.toFixed(1)}% vs previous period`, tone: 'text-[var(--dashboard-success)]' }
      : view.change < 0
        ? { text: `${view.change.toFixed(1)}% vs previous period`, tone: 'text-[var(--dashboard-danger)]' }
        : { text: 'No change vs previous period', tone: 'text-[var(--dashboard-muted)]' }
    ...
    <article className="flex min-h-[354px] flex-col overflow-hidden rounded-2xl border border-[var(--dashboard-border)] bg-[var(--dashboard-surface)] shadow-[0_1px_2px_rgba(16,24,40,.04)]">
      <div className="flex items-start justify-between gap-3 border-b border-[var(--dashboard-border)] px-5 py-3.5">
        ...
        <div className="relative shrink-0">
          ...
          {rangeOpen && (
            <div role="listbox" className="absolute right-0 top-[calc(100%+4px)] z-30 min-w-full overflow-hidden rounded-lg border border-[var(--dashboard-border)] bg-[var(--dashboard-surface)] p-1 shadow-lg">
              {RANGE_OPTIONS.map((option) => (
                <button
                  ...
                  onClick={() => { setRange(option.value); setRangeOpen(false) }}
      ...
      <div className="flex flex-1 flex-col px-5 pb-3 pt-3.5">
        ...
        <div className="mt-3 h-[126px] min-h-[126px] w-full" role="img" aria-label="Revenue trend over the selected period">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={view.points} margin={{ top: 8, right: 4, bottom: 0, left: -18 }}>
              <CartesianGrid vertical={false} stroke="var(--dashboard-chart-grid)" strokeDasharray="3 4" />
        ...
        <dl className="mt-auto grid grid-cols-3 divide-x divide-[var(--dashboard-border)] border-t border-[var(--dashboard-border)] pt-3">
          <div className="pr-3">
            <dt className="text-[0.62rem] font-medium text-[var(--dashboard-muted)]">Revenue</dt>
            <dd className="mt-0.5 truncate text-xs font-bold tabular-nums text-[var(--dashboard-text)]">{compactCurrency(view.revenue, currency)}</dd>
          </div>
          <div className="px-3">
            <dt className="text-[0.62rem] font-medium text-[var(--dashboard-muted)]">Completed sales</dt>
            <dd className="mt-0.5 text-xs font-bold tabular-nums text-[var(--dashboard-text)]">{formatNumber(view.transactions)}</dd>
          </div>
          <div className="pl-3">
            <dt className="text-[0.62rem] font-medium text-[var(--dashboard-muted)]">Average transaction</dt>
            <dd className="mt-0.5 truncate text-xs font-bold tabular-nums text-[var(--dashboard-text)]">{compactCurrency(view.average, currency)}</dd>
          </div>
        </dl>
      </div>
    </article>
