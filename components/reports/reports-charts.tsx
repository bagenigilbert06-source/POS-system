...
function total(
  points: TrendPoint[],
  key: 'revenue' | 'expenses' | 'refunds' | 'netProfit' | 'count'
) {
...
export function ReportsCharts({
  ...
  const hasActivity = chartData.some(
    (point) => point.revenue || point.expenses || point.refunds
  );
  ...
  return (
    <section
      className={`grid gap-4 ${showTrend && showPayment ? 'xl:grid-cols-[minmax(0,1.45fr)_minmax(360px,.75fr)]' : ''}`}
    ...
    >
        ...
        <article className="app-panel report-paint-boundary overflow-hidden">
          <div className="flex flex-col gap-4 border-b px-4 py-4 sm:flex-row sm:items-start sm:justify-between sm:px-5">
            <div className="flex items-center gap-2">
              <span className="flex h-8 w-8 items-center justify-center rounded-lg border border-[var(--dashboard-accent-soft-border)] bg-[var(--dashboard-accent-soft)] text-[var(--dashboard-accent)]">
                <TrendingUp className="h-4 w-4" aria-hidden="true" />
              </span>
              <div>
                <h2>{useMonthlyView ? 'Monthly sales' : 'Daily sales'}</h2>
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Each bar shows the net sales collected{' '}
                  {useMonthlyView ? 'that month' : 'that day'}.
                </p>
              </div>
            </div>
            <span className="rounded-lg border bg-[var(--dashboard-surface-subtle)] px-3 py-2 text-[11px] font-semibold text-muted-foreground">
              {periodLabel}
            </span>
          </div>
          ...
          {hasActivity ? (
            <>
              <p className="sr-only">
                {periodLabel}: net sales {formatCurrency(revenue, currency)},
                expenses {formatCurrency(expenses, currency)}, refunds{' '}
              ...
              <div className="flex flex-wrap items-center gap-x-5 gap-y-2 border-t px-5 py-3 text-[11px] font-medium text-muted-foreground">
                <span>
                  <strong className="text-foreground">{activePeriods}</strong>{' '}
                  {useMonthlyView ? 'months' : 'days'} with sales
                </span>
                {expenses > 0 && (
                  <span>
                    Expenses:{' '}
            ...
            <ChartEmpty
              ...
              detail="Sales and expenses will appear here as they are recorded."
            />
          )}
        </article>
      )}
      ...
      {showPayment && (
        <article className="app-panel report-paint-boundary overflow-hidden">
          <div className="flex items-start justify-between gap-4 border-b px-4 py-4 sm:px-5">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Collected sales by payment method.
              </p>
            </div>
          ...
          {paymentChartData.length && paymentTotal > 0 ? (
            <div className="flex min-h-[300px] flex-col p-5">
              <div className="relative mx-auto h-[210px] w-full max-w-[270px]">
                ...
                <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center text-center">
                  ...
                  <span className="mt-1 max-w-[118px] truncate text-sm font-bold tabular-nums">
                    {compact(paymentTotal)}
                  </span>
                </div>
              </div>
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      <p className="mt-0.5 text-[10px] font-semibold text-muted-foreground tabular-nums">
                        {paymentShareLabel(item.amount, paymentTotal)}
                      </p>
                    </div>
                  </div>
                ))}
              ...
              {primaryPayment && (
                <p className="mt-auto rounded-lg border bg-muted/25 px-3 py-2.5 text-xs leading-5 text-muted-foreground">
                  <span className="font-semibold text-foreground">
                    {primaryPayment.label}
                  </span>{' '}
                  leads at{' '}
                  {paymentShareLabel(primaryPayment.amount, paymentTotal)} of
                  collected sales.
                </p>
              )}
            </div>
            ...
            <ChartEmpty
              ...
              detail="Payment methods will be compared after a recorded sale."
            />
          )}
        </article>
      )}
  ...
  return (
    <div className="min-w-0 px-4 py-3 sm:px-5">
      ...
      <div className="mt-1 flex items-center gap-2">
        <p
          className={`truncate text-xs font-bold tabular-nums ${tone === 'positive' ? 'text-emerald-700 dark:text-emerald-400' : tone === 'negative' ? 'text-rose-600' : ''}`}
        >
          {value}
        </p>
        {trend != null && (
          <span
            className={`text-[10px] font-bold tabular-nums ${trend >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-600'}`}
          >
            {trend >= 0 ? '+' : ''}
            {trend.toFixed(1)}%
          </span>
...
function TrendTooltip({
  ...
  return (
    <div className="min-w-48 rounded-xl border border-[var(--dashboard-border)] bg-[var(--dashboard-chart-tooltip)] p-3 text-xs shadow-xl">
      <p className="font-bold">{point.label}</p>
      <div className="mt-2 space-y-1.5">
        <TooltipRow
          label="Net sales"
          value={formatCurrency(point.revenue, currency)}
        />
        <TooltipRow
          label="Expenses"
          value={formatCurrency(point.expenses, currency)}
        />
        <TooltipRow
          label="Refunds"
          value={formatCurrency(point.refunds, currency)}
        />
        <TooltipRow label="Transactions" value={formatNumber(point.count)} />
      </div>
    </div>
  );
}
...
function PaymentTooltip({
  ...
  payload?: ReadonlyArray<{
    payload?: { label: string; amount: number; transactions: number };
  }>;
  currency: string;
  total: number;
}) {
  ...
  if (!active || !point) return null;
  return (
    <div className="min-w-44 rounded-xl border border-[var(--dashboard-border)] bg-[var(--dashboard-chart-tooltip)] p-3 text-xs shadow-xl">
      <p className="font-bold">{point.label}</p>
      <p className="mt-2 font-bold tabular-nums">
        {formatCurrency(point.amount, currency)}
      </p>
      <p className="mt-1 text-muted-foreground">
        {formatNumber(point.transactions)} transactions ·{' '}
        {paymentShareLabel(point.amount, paymentTotal)}
      </p>
    </div>
  );
}
