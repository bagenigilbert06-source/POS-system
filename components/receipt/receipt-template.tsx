...
export function ReceiptTemplate({
  ...
  // Receipt branding is controlled only by the chosen visual template. Tax/VAT
  // settings must never decide whether a saved logo is rendered or printed.
  const effectiveLogoUrl = receiptLogoForTemplate(template, logoUrl)
  ...
  const thermalReceiptData = encodeThermalReceiptModel({
    ...
    qrCodes: [
      ...(sale.feedbackUrl ? [{ value: sale.feedbackUrl, label: 'Scan to share your feedback' }] : []),
  ...
  if (layout === 'thermal') {
    return <div style={{ fontFamily: 'Manrope, Inter, Arial, sans-serif' }} className="receipt-paper receipt-thermal receipt-java-style mx-auto w-full max-w-full bg-white px-[3mm] py-[3mm] text-[9pt] font-medium leading-[1.35] text-black print:w-full print:max-w-none print:px-[2mm] print:py-[2mm]" {...{ [THERMAL_RECEIPT_DATA_ATTRIBUTE]: thermalReceiptData }}>
      ...
      {sale.feedbackUrl && <div className="mt-4 border-y border-dotted border-zinc-700 py-3 text-center"><p className="font-bold">HOW DID WE DO?</p><p className="mt-1 text-[9px]">Scan to share your feedback</p><div className="mt-2 flex justify-center"><ReceiptQrCode saleId={sale.id} receiptNo={sale.receiptNo} total="" paymentMethod="" createdAt={sale.createdAt} value={sale.feedbackUrl} label="Scan to share your feedback" size={112} /></div></div>}
  ...
  return (
    <div className="mx-auto w-full max-w-xl font-sans text-sm text-[#111827] print:p-0 print:bg-white print:text-black">
      ...
      <div className="receipt receipt-paper overflow-hidden rounded-2xl border border-zinc-200 bg-white text-black shadow-[0_12px_32px_rgba(15,23,42,.08)] print:rounded-none print:border-0 print:shadow-none" {...{ [THERMAL_RECEIPT_DATA_ATTRIBUTE]: thermalReceiptData }}>
        ...
        <div className="flex items-start justify-between gap-6 border-b border-zinc-200 px-6 py-5 sm:px-7">
          <div>
            <div className="flex items-center gap-3">
              {effectiveLogoUrl ? <span className="flex h-11 w-28 items-center justify-center rounded-lg bg-zinc-50"><Image src={effectiveLogoUrl} alt={`${businessName} logo`} width={112} height={44} unoptimized className="h-11 w-28 object-contain p-1" /></span> : null}
        ...
        {sale.feedbackUrl && <div className="mx-6 mt-5 rounded-xl border border-zinc-200 p-4 text-center sm:mx-7"><p className="text-xs font-bold">HOW DID WE DO?</p><p className="mt-1 text-xs text-zinc-500">Scan to share your feedback</p><div className="mt-2 flex justify-center"><ReceiptQrCode saleId={sale.id} receiptNo={sale.receiptNo} total="" paymentMethod="" createdAt={sale.createdAt} value={sale.feedbackUrl} label="Scan to share your feedback" /></div></div>}
