...
export async function closePosSession(closingCash: number, notes = '') { const counted = z.coerce.number().nonnegative().max(999999999).parse(closingCash); const authorization = await posOperator(PermissionEnum.SHIFT_CLOSE); const { userId, orgId } = authorization; const [current] = await db.select().from(posSession).where(and(eq(posSession.orgId, orgId), eq(posSession.status, 'open'), authorization.permissions.includes(PermissionEnum.SHIFT_MANAGE) ? undefined : eq(posSession.openedBy, userId))).orderBy(desc(posSession.openedAt)).limit(1); if (!current) throw new Error('No open register'); const [[cashSales], [cashRefunds], [moves]] = await Promise.all([db.select({ total: sql<string>`coalesce(sum(${sale.total}),0)` }).from(sale).where(and(eq(sale.orgId, orgId), inArray(sale.status, ['completed', 'partially_refunded', 'refunded']), eq(sale.paymentMethod, 'cash'), gte(sale.createdAt, current.openedAt), authorization.permissions.includes(PermissionEnum.SHIFT_MANAGE) ? undefined : eq(sale.userId, userId))), db.select({ total: sql<string>`coalesce(sum(${salesReturn.amount}),0)` }).from(salesReturn).where(and(eq(salesReturn.orgId, orgId), eq(salesReturn.status, 'completed'), eq(salesReturn.refundMethod, 'cash'), gte(salesReturn.createdAt, current.openedAt), authorization.permissions.includes(PermissionEnum.SHIFT_MANAGE) ? undefined : eq(salesReturn.userId, userId))), db.select({ total: sql<string>`coalesce(sum(case when ${cashMovement.type}='cash_in' then ${cashMovement.amount} else -${cashMovement.amount} end),0)` }).from(cashMovement).where(eq(cashMovement.sessionId, current.id))]); const expected = Number(current.openingCash) + Number(cashSales.total) - Number(cashRefunds.total) + Number(moves.total), variance = counted - expected; const [closed] = await db.update(posSession).set({ status: 'closed', expectedCash: String(expected), closingCash: String(counted), variance: String(variance), notes: notes || null, closedBy: userId, closedAt: new Date() }).where(and(eq(posSession.id, current.id), eq(posSession.orgId, orgId), eq(posSession.status, 'open'))).returning({ id: posSession.id }); if (!closed) throw new Error('This register has already been closed'); await db.insert(auditEvent).values({ id: generateId(), organizationId: orgId, userId, action: 'shift.reconciled', metadata: { sessionId: current.id, expectedCash: expected, countedCash: counted, variance, cashRefunds: Number(cashRefunds.total) } }); refresh(); return { expectedCash: expected, countedCash: counted, variance } }
