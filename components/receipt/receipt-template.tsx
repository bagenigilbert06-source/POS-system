...
export function ReceiptTemplate({
  ...
  if (layout === 'thermal') {
    ...
    return <div className="receipt-paper receipt-thermal mx-auto w-full max-w-full bg-white px-[3mm] py-[4mm] font-['Courier_New',Courier,monospace] text-[11px] leading-[1.5] text-black print:w-full print:max-w-none print:px-[2mm] print:py-[3mm]">
      ...
      <div className="my-[3mm] border-y border-dashed border-black py-[2mm] text-center"><p className="font-bold tracking-wide">{isProvisional ? 'PROVISIONAL RECEIPT' : 'SALES RECEIPT'}</p><p>{formatDateTime(sale.createdAt)}</p><p>Receipt: {sale.receiptNo}</p>{showCashier && <p>Cashier: {cashierName}</p>}{showCustomer && <p>Customer: {customerName}</p>}{isProvisional && <><p className="mt-2 border-y border-black py-1 font-black">OFFLINE · SYNC PENDING</p><p className="mt-1 text-[9px] font-bold leading-3">NOT AN OFFICIAL OR FISCAL RECEIPT</p></>}</div>
