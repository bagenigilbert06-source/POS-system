...
async function seedWorkspace() {
  const [ownerHash, maryHash, janeHash] = await Promise.all([
    ...
    hashPassword(maryPin),
    hashPassword(janePin),
  ]);
  await pool.query('BEGIN');
  try {
    await pool.query(
      `INSERT INTO "user" (id,name,email,"emailVerified") VALUES ($1,'E2E Owner',$2,true),($3,'Mary Cashier',$4,true),($5,'Jane Cashier',$6,true)`,
      [
        ids.owner,
        ownerEmail,
        ids.mary,
        `e2e-mary-${run}@example.invalid`,
        ids.jane,
        `e2e-jane-${run}@example.invalid`,
      ]
    );
    await pool.query(
      `INSERT INTO account (id,"accountId","providerId","userId",password) VALUES ($1,$2,'credential',$2,$3)`,
      [`e2e-account-${run}`, ids.owner, ownerHash]
    );
    await pool.query(
      `INSERT INTO organization (id,name,slug,"businessType","businessCategory","userId","onboardingCompleted",timezone) VALUES ($1,'E2E Shop',$2,'retail','general_shop',$3,true,'Africa/Nairobi')`,
      [ids.org, `e2e-${run}`, ids.owner]
    );
    await pool.query(
      `INSERT INTO organization_membership (id,"organizationId","userId",role) VALUES ($1,$2,$3,'owner'),($4,$2,$5,'cashier'),($6,$2,$7,'cashier')`,
      [
        `e2e-owner-member-${run}`,
        ids.org,
        ids.owner,
        `e2e-mary-member-${run}`,
        ids.mary,
    ...
    await pool.query(
      `INSERT INTO branch (id,"organizationId",code,name,"isMain") VALUES ($1,$2,'E2E','E2E Main Branch',true)`,
      [ids.branch, ids.org]
    );
    await pool.query(
      `INSERT INTO branch_membership (id,"branchId","userId",role) VALUES ($1,$2,$3,'owner'),($4,$2,$5,'cashier'),($6,$2,$7,'cashier')`,
      [
        `e2e-owner-branch-${run}`,
        ids.branch,
        ids.owner,
        `e2e-mary-branch-${run}`,
        ids.mary,
    ...
    await pool.query(
      `INSERT INTO employee (id,"userId",name,email,role,status,"orgId") VALUES ($1,$2,'E2E Owner',$3,'owner','active',$4),($5,$6,'Mary Cashier',$7,'cashier','active',$4),($8,$9,'Jane Cashier',$10,'cashier','active',$4)`,
      [
        `e2e-owner-employee-${run}`,
        ids.owner,
        ownerEmail,
        ids.org,
        `e2e-mary-employee-${run}`,
        ids.mary,
        `e2e-mary-${run}@example.invalid`,
        `e2e-jane-employee-${run}`,
        ids.jane,
        `e2e-jane-${run}@example.invalid`,
      ]
    );
    await pool.query(
      `INSERT INTO workspace (id,"organizationId",config) VALUES ($1,$2,$3)`,
      [
        `e2e-workspace-${run}`,
        ids.org,
        {
          templateId: 'retail-general_shop',
          enabledModules: [
            'pos',
            'sales',
            'products',
          ...
          businessCategory: 'general_shop',
        },
      ]
    );
    await pool.query(
      `INSERT INTO business_settings ("organizationId","displayName","paymentMethods","defaultPaymentMethod","taxEnabled","taxRate","receiptBusinessName") VALUES ($1,'E2E Shop',$2,'cash',false,0,'E2E Shop')`,
      [ids.org, JSON.stringify(['cash'])]
    );
    await pool.query(
      `INSERT INTO pos_pin_credential ("userId","pinHash",enabled) VALUES ($1,$2,true),($3,$4,true)`,
      [ids.mary, maryHash, ids.jane, janeHash]
    );
    await pool.query(
      `INSERT INTO product (id,name,sku,"sellingPrice",stock,"minStock",unit,"userId","orgId","isActive") VALUES ($1,'E2E Test Item','E2E-ITEM',100,20,2,'pcs',$2,$3,true)`,
      [ids.product, ids.owner, ids.org]
    );
    await pool.query(
      `INSERT INTO inventory_balance (id,"productId","branchId","onHand",reserved,unavailable,incoming,"orgId") VALUES ($1,$2,$3,20,0,0,0,$4)`,
      [`e2e-balance-${run}`, ids.product, ids.branch, ids.org]
    );
...
async function terminalOnlyContext(source: BrowserContext, browser: Browser) {
  const terminal = (await source.cookies()).filter(
    (cookie) => cookie.name === 'pesaby_pos_terminal'
  );
...
test('cashier journey, unresolved-shift blocks, stale-session protection and Mary to Jane handoff', async ({
  page,
  context,
  browser,
}) => {
  ...
  await expect(
    page.getByRole('button', { name: /register this device/i })
  ).toBeHidden();
  await pool.query(
    `UPDATE pos_terminal SET "printingMode"='direct', "printerDisplayName"='E2E Persistent Printer', "printerIdentifier"='E2E Persistent Printer', "paperWidth"=80, "autoPrint"=false, "cashDrawerPulse"=false WHERE "organizationId"=$1`,
    [ids.org]
  );
  await page.reload();
  await expect(page.getByTitle('E2E Persistent Printer')).toBeVisible();
  ...
  await page
    .getByRole('button', { name: /add e2e test item to basket/i })
    .click();
  await expect(
    page.getByRole('button', { name: 'Start shift to take payment' })
  ).toBeDisabled();
  ...
  await expect(
    page.getByRole('heading', { name: 'Count the drawer' })
  ).toBeVisible();
  ...
  await page
    ...
    .fill('200');
  await page
    .getByRole('button', { name: 'Continue to reconciliation' })
    .click();
  ...
  await expect
    ...
    .toBe(true);
  const persistedPrinter = await pool.query(
    `SELECT "printingMode", "printerIdentifier", "paperWidth", "autoPrint", "cashDrawerPulse"
       FROM pos_terminal WHERE "organizationId"=$1`,
    [ids.org]
  );
  expect(persistedPrinter.rows[0]).toMatchObject({
    printingMode: 'direct',
    printerIdentifier: 'E2E Persistent Printer',
    paperWidth: 80,
    autoPrint: false,
...
test('manager can recover and close Jane shift', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', { name: 'Manager shift recovery' })
  ).toBeVisible();
  ...
  await expect(
    page.getByText('Shift reconciled and terminal released')
  ).toBeVisible();
});
