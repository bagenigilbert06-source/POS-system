...
export function PosPinLoginForm() {
  ...
  const submit=(value=pin)=>{if(value.length!==6||pending)return;start(async()=>{setError('');const result=await unlockPosByPin(value);if('error' in result){setPin('');setError(result.error.includes('currently has an open shift') || result.error.includes('registered to a different store') ? result.error : 'PIN couldn\'t unlock this POS. Check the PIN or ask a manager.');requestAnimationFrame(()=>inputRef.current?.focus());return;}router.replace('/dashboard/pos');router.refresh()})};
  ...
  return <div className="w-full space-y-2.5">
    <label htmlFor="pos-pin" className="text-sm font-semibold text-zinc-800">POS PIN</label>
    <button type="button" onClick={()=>inputRef.current?.focus()} className="relative flex h-12 w-full items-center justify-center rounded-lg border border-[#d4aa00] bg-white px-4 ring-1 ring-[#ffda32]/20 focus-within:border-[#d4aa00] focus-within:ring-2 focus-within:ring-[#ffda32]/30" aria-label="Enter six digit PIN">
    ...
    <div className="grid grid-cols-3 gap-1.5" aria-label="PIN keypad">{keys.map(key=><button key={key} type="button" disabled={pending} onClick={()=>{press(key);inputRef.current?.focus()}} className="flex h-11 items-center justify-center rounded-lg border border-zinc-200 bg-white text-base font-bold text-zinc-800 transition-colors hover:border-[#d4aa00] hover:bg-[#fff9df] active:scale-[.98] disabled:opacity-50">{key==='clear'?<span className="text-xs">Clear</span>:key==='delete'?<Delete className="h-4 w-4"/>:key}</button>)}</div>
    ...
    <button type="button" onClick={()=>submit()} disabled={pin.length!==6||pending} aria-label={pending ? 'Unlocking POS' : 'Unlock POS'} className="flex h-12 w-full items-center justify-center rounded-lg bg-[#ffda32] text-sm font-extrabold text-slate-950 transition-colors hover:bg-[#e8c42d] disabled:bg-[#fff3b8] disabled:text-slate-500">{pending ? <Loader2 label="Unlocking POS" className="h-8 w-8" style={{ color: '#f45113' }} /> : <span className="flex items-center justify-center gap-2.5"><LockKeyhole className="h-5 w-5 shrink-0" strokeWidth={2.25} aria-hidden="true" /><span className="leading-none">Unlock POS</span></span>}</button>
    {(context.terminalName||context.branchName)&&<div className="-mt-1 text-center text-[11px] text-zinc-400">{[context.terminalName,context.branchName].filter(Boolean).join(' • ')}</div>}
  </div>;
}
