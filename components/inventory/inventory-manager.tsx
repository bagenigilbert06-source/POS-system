...
import type { Branch, InventoryBalance, Product, StockAdjustment, StockAdjustmentItem, StockMovement } from '@/lib/db/schema'
...
type InventoryProduct = Product & { unitsSoldMonth: number; onHand?: number; reserved?: number; unavailable?: number; incoming?: number }
...
export function InventoryManager({ products: organizationProducts, movements, adjustments, adjustmentItems, balances, branches, currency, canAdjust, canPurchase }: InventoryManagerProps) {
  ...
  const [countLines, setCountLines] = useState<Array<{ productId: string; quantityAfter: number }>>([])
  const [reorderProduct, setReorderProduct] = useState<InventoryProduct | null>(null)
  const [branchId, setBranchId] = useState(branches[0]?.id ?? '')
  const locationStock = useMemo(() => new Map(balances.filter((item) => item.branchId === branchId).map((item) => [item.productId, item])), [balances, branchId])
  const products = useMemo(() => organizationProducts.map((item) => { const balance = locationStock.get(item.id); const onHand = Number(balance?.onHand ?? 0), reserved = Number(balance?.reserved ?? 0), unavailable = Number(balance?.unavailable ?? 0); return { ...item, minStock: Number(balance?.reorderPoint ?? item.minStock), stock: Math.max(0, onHand - reserved - unavailable), onHand, reserved, unavailable, incoming: Number(balance?.incoming ?? 0) } }), [organizationProducts, locationStock])
  const openCount = (item: InventoryProduct) => { setCountProduct(item); setCountLines([{ productId: item.id, quantityAfter: item.onHand ?? item.stock }]) }
  ...
  const replenishment = products.filter((item) => item.stock + (item.incoming ?? 0) <= item.minStock).sort((a, b) => (a.stock - a.minStock) - (b.stock - b.minStock))
  ...
  return <>
    <section className="overflow-hidden rounded-2xl border bg-card shadow-sm">
      ...
      <div className="flex flex-col gap-3 border-b p-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {branches.length > 1 && <Select value={branchId} onValueChange={setBranchId}><SelectTrigger className="h-10 w-[190px]"><SelectValue placeholder="Inventory location" /></SelectTrigger><SelectContent>{branches.map((item) => <SelectItem key={item.id} value={item.id}>{item.name}</SelectItem>)}</SelectContent></Select>}
          ...
          {canAdjust && tab !== 'movements' && <Button type="button" onClick={() => products[0] && openCount(products[0])} disabled={!products.length} className="h-10 gap-2"><ClipboardCheck className="h-4 w-4" />Count stock</Button>}
        </div>
      </div>

      {tab === 'stock' && <StockTable products={visibleProducts} currency={currency} statusOf={statusOf} canAdjust={canAdjust} onCount={openCount} onReorder={setReorderProduct} />}
    ...
    <Dialog open={Boolean(countProduct)} onOpenChange={(open) => !open && setCountProduct(null)}>
      <DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>Record a physical stock count</DialogTitle><DialogDescription>Enter what was physically counted. The variance remains pending until reviewed and approved.</DialogDescription></DialogHeader>
        {countProduct && <form action={(form) => run(() => createStockAdjustment({ branchId, type: String(form.get('type')) as 'stocktake' | 'loss' | 'damage' | 'correction', items: countLines, notes: String(form.get('notes')) }), 'Stock count submitted for approval', () => setCountProduct(null))} className="space-y-4 pt-2">
          <div className="space-y-2"><Label>Products counted</Label>{countLines.map((line, index) => { const selected = products.find((item) => item.id === line.productId); return <div key={`${line.productId}-${index}`} className="grid grid-cols-[1fr_110px_36px] gap-2"><Select value={line.productId} onValueChange={(id) => { const item = products.find((product) => product.id === id); setCountLines((current) => current.map((entry, lineIndex) => lineIndex === index ? { productId: id, quantityAfter: item?.onHand ?? item?.stock ?? 0 } : entry)) }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{products.filter((item) => item.id === line.productId || !countLines.some((entry) => entry.productId === item.id)).map((item) => <SelectItem key={item.id} value={item.id}>{item.name} · {item.onHand ?? item.stock} {item.unit} on hand</SelectItem>)}</SelectContent></Select><Input aria-label={`Physical quantity for ${selected?.name ?? 'product'}`} type="number" min="0" max="10000000" value={line.quantityAfter} onChange={(event) => setCountLines((current) => current.map((entry, lineIndex) => lineIndex === index ? { ...entry, quantityAfter: Number(event.target.value) } : entry))} required /><button type="button" aria-label="Remove count line" disabled={countLines.length === 1} onClick={() => setCountLines((current) => current.filter((_, lineIndex) => lineIndex !== index))} className="rounded-md text-muted-foreground hover:bg-muted disabled:opacity-30"><X className="mx-auto h-4 w-4" /></button></div> })}<Button type="button" variant="outline" size="sm" disabled={countLines.length >= products.length} onClick={() => { const next = products.find((item) => !countLines.some((line) => line.productId === item.id)); if (next) setCountLines((current) => [...current, { productId: next.id, quantityAfter: next.onHand ?? next.stock }]) }}>Add another product</Button></div>
          <div className="space-y-2"><Label>Count reason</Label><Select name="type" defaultValue="stocktake" required><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="stocktake">Cycle count</SelectItem><SelectItem value="correction">Correction</SelectItem><SelectItem value="damage">Damage</SelectItem><SelectItem value="loss">Loss</SelectItem></SelectContent></Select></div>
    ...
    <Dialog open={Boolean(reorderProduct)} onOpenChange={(open) => !open && setReorderProduct(null)}>
      <DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>Update reorder level</DialogTitle><DialogDescription>Pesaby flags this item when on-hand stock reaches this quantity.</DialogDescription></DialogHeader>
        {reorderProduct && <form action={(form) => run(() => updateReorderLevel({ productId: reorderProduct.id, branchId, minStock: Number(form.get('minStock')) }), 'Reorder level updated', () => setReorderProduct(null))} className="space-y-4 pt-2"><div className="rounded-lg border bg-muted/20 px-4 py-3"><p className="font-semibold">{reorderProduct.name}</p><p className="mt-1 text-xs text-muted-foreground">Currently {reorderProduct.onHand ?? reorderProduct.stock} {reorderProduct.unit} on hand at this location</p></div><div className="space-y-2"><Label htmlFor="reorder-level">Low-stock alert level</Label><Input id="reorder-level" name="minStock" type="number" min="0" max="10000000" defaultValue={reorderProduct.minStock} required /></div><Button disabled={pending} className="w-full">{pending ? 'Saving…' : 'Save reorder level'}</Button></form>}
      </DialogContent>
    </Dialog>
  </>
}
...
function Replenishment({ products, currency, canPurchase, canAdjust, onReorder }: { products: InventoryProduct[]; currency: string; canPurchase: boolean; canAdjust: boolean; onReorder: (item: InventoryProduct) => void }) {
  ...
  return <div><div className="border-b bg-amber-50/60 px-5 py-4 text-sm dark:bg-amber-950/10"><p className="font-semibold text-amber-900 dark:text-amber-200">Suggested quantities use available stock, confirmed incoming stock, the safety level and recent 30-day demand.</p><p className="mt-1 text-xs text-amber-800/80 dark:text-amber-300/70">Review supplier pack sizes and expected demand before ordering.</p></div><div className="divide-y">{products.map((item) => { const projected = item.stock + (item.incoming ?? 0); const suggested = recommendedOrderQuantity(projected, item.minStock, item.unitsSoldMonth); return <article key={item.id} className="flex flex-col gap-3 px-5 py-4 md:flex-row md:items-center"><span className={cn('h-2.5 w-2.5 shrink-0 rounded-full', item.stock <= 0 ? 'bg-red-500' : 'bg-amber-500')} /><div className="min-w-0 flex-1"><p className="font-semibold">{item.name}</p><p className="mt-1 text-xs text-muted-foreground">{item.stock} {item.unit} available · {item.incoming ?? 0} incoming · alert at {item.minStock} · {item.unitsSoldMonth} sold this month · {formatCurrency(Number(item.buyingPrice) * suggested, currency)} estimated cost</p></div><div className="flex items-center gap-4"><div className="text-right"><p className="text-xs text-muted-foreground">Suggested order</p><p className="font-bold tabular-nums">{suggested} {item.unit}</p></div>{canAdjust && <Button variant="outline" size="sm" onClick={() => onReorder(item)}>Edit level</Button>}{canPurchase && <Button asChild size="sm"><Link href={`/dashboard/purchases?productId=${item.id}`}>Receive stock</Link></Button>}</div></article>})}</div></div>
}
