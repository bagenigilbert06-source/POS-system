        ...
        <section className="hidden border-r border-black/10 bg-[#ffda32] px-14 py-8 text-slate-950 lg:flex lg:flex-col">
          ...
          <div className="flex flex-1 items-center">
            <div className="w-full max-w-xl">
              <p className="mb-4 text-xs font-extrabold uppercase tracking-[0.18em] text-[#e42527]">{isPosLogin ? 'Counter access' : 'Welcome back'}</p>
              <h2 className="max-w-lg text-5xl font-extrabold leading-[1.02] tracking-[-0.045em] text-slate-950">
                {isPosLogin ? 'Secure access for every member of your team.' : 'Your business, ready when you are.'}
              </h2>
              <p className="mt-5 max-w-md text-base leading-7 text-slate-700">
                {isPosLogin ? 'Cashiers can unlock this registered terminal with a phone and PIN. Managers and owners can also use their account password for complete dashboard access.' : 'Sign in to manage sales, inventory, employees, customers, expenses and business performance from one secure workspace.'}
              </p>
              ...
              <p className="mt-6 text-sm font-semibold text-slate-800">{isPosLogin ? 'PIN protects counter operations; passwords protect full workspace access.' : 'Secure access to everything you need to run your business.'}</p>
            </div>
          </div>
          ...
          <div className="w-full max-w-[440px] border-0 bg-transparent p-0 shadow-none sm:p-8">
            ...
            <div className="mb-8">
              <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-lg bg-[#ffda32] text-slate-950">
                {isPosLogin ? <IconReceipt className="h-5 w-5" stroke={1.8} aria-hidden="true" /> : <IconLockAccess className="h-5 w-5" stroke={1.8} aria-hidden="true" />}
              </div>
              <h1 className="mb-2 text-3xl font-extrabold tracking-[-0.035em] text-slate-950">{isPosLogin ? 'Sign in to Pesaby' : 'Welcome back'}</h1>
              <p className="text-sm leading-6 text-zinc-600">{isPosLogin ? 'Choose the secure method that matches the work you need to do.' : 'Sign in to continue to your Pesaby workspace.'}</p>
            </div>
