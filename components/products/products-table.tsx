      ...
      <div className="space-y-4">
        ...
        {filtered.length > 0 && viewMode === 'grid' && <div className="grid grid-cols-[repeat(auto-fill,minmax(260px,300px))] justify-start gap-3">
          {filtered.map((p) => {
            ...
            return <article key={`card-${p.id}`} className="group relative h-full w-full max-w-[300px] min-w-0 overflow-hidden rounded-md border border-[#d9e0ea] bg-white text-sm transition hover:border-[#d8b92e] hover:shadow-[0_8px_20px_rgba(16,24,40,.08)]">
              <div className="relative overflow-hidden bg-[#fff8e8]">
                <Link href={`/dashboard/products/${p.id}`} className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#e42527]" aria-label={`View ${p.name} details`}>
                  {p.imageUrl ? <Image src={p.imageUrl} alt={`${p.name} preview`} width={320} height={220} unoptimized onClick={(event) => { event.preventDefault(); event.stopPropagation(); setZoomedImage({ src: p.imageUrl!, name: p.name }) }} className={`h-36 w-full cursor-zoom-in object-cover transition-transform duration-500 sm:h-40 lg:h-44 ${p.stock !== 0 ? 'group-hover:scale-105' : 'opacity-50'}`} /> : <div className="flex h-36 items-center justify-center text-[#8a6500] sm:h-40 lg:h-44"><Package className="h-12 w-12 stroke-[1.35]" /></div>}
                </Link>
              ...
              <Link href={`/dashboard/products/${p.id}`} className="flex min-w-0 flex-col gap-2.5 p-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#e42527]" aria-label={`View ${p.name} details`}>
                ...
                <div className="flex items-center justify-between border-t border-[#edf0f4] pt-3 text-xs"><span className="text-muted-foreground">{p.stock} {p.unit} available</span><span className="text-muted-foreground">{p.unitsSoldMonth} sold this month</span></div>
